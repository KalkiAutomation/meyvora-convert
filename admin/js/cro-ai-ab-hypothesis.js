(function ($) {
	...
	var cfg =
		typeof croAiAbHypothesis !== "undefined" ? croAiAbHypothesis : null;
	...
	function renderDiffRow(label, before, after, s) {
		...
		var $row = $("<div class='cro-ai-ab-diff-row'></div>");
		$row.append(
			$("<div class='cro-ai-ab-diff-label'></div>").text(label)
		);
		var $cols = $("<div class='cro-ai-ab-diff-cols'></div>");
		$cols.append(
			$("<div class='cro-ai-ab-diff-col cro-ai-ab-diff-col--before'></div>")
				.append(
					$("<span class='cro-ai-ab-diff-tag'></span>").text(
						(s && s.before) || ""
					)
				)
				.append($("<div class='cro-ai-ab-diff-text'></div>").text(b))
		);
		$cols.append(
			$("<div class='cro-ai-ab-diff-col cro-ai-ab-diff-col--after'></div>")
				.append(
					$("<span class='cro-ai-ab-diff-tag'></span>").text(
						(s && s.after) || ""
					)
				)
				.append($("<div class='cro-ai-ab-diff-text'></div>").text(a))
		);
		$row.append($cols);
		return $row;
	}
	...
	function renderCards($inner, campaignId, variants, baseline) {
		...
		var bl = baseline || {};
		$inner.empty();
		var $grid = $("<div class='cro-ai-ab-cards'></div>");
		variants.forEach(function (v) {
			var $card = $("<div class='cro-ai-ab-card'></div>");
			$card.append($("<h4 class='cro-ai-ab-card__title'></h4>").text(v.name || ""));
			$card.append(
				$("<p class='cro-ai-ab-card__meta'></p>").html(
					"<strong>" +
			...
			$card.append(
				$("<p class='cro-ai-ab-card__hypothesis'></p>").html(
					"<strong>" +
			...
			$card.append(
				$("<p class='cro-ai-ab-card__summary'></p>").html(
					"<strong>" +
			...
			var $diff = $("<div class='cro-ai-ab-diff'></div>");
			$diff.append(
			...
			var $btn = $("<a class='button button-primary'></a>")
				...
				.text(s.startTest || "");
			$card.append($("<p class='cro-ai-ab-card__actions'></p>").append($btn));
			$grid.append($card);
		});
		$inner.append($grid);
	}
	...
	function showConfigureOnly($inner) {
		...
		$inner.empty();
		$inner.append(
			$("<p class='cro-ai-ab-panel__notice description'></p>").text(
				s.configure || ""
			)
		);
	}
	...
	function loadVariants($tr, campaignId) {
		var $inner = $tr.find(".cro-ai-hypothesis-panel__inner");
		if (!cfg || !cfg.aiReady) {
			showConfigureOnly($inner);
			return;
		}
		$inner.html(
			"<p class='cro-ai-ab-panel__loading'><span class='spinner is-active' style='float:none;margin:0 8px 0 0;'></span>" +
	...
	$(function () {
		$(document).on("click", ".js-cro-ai-ab-hypothesis-toggle", function (e) {
			e.preventDefault();
			var id = $(this).data("campaign-id");
			var $panel = $("#cro-ai-hypothesis-panel-" + id);
			if (!$panel.length) {
				return;
			}
			var $inner = $panel.find(".cro-ai-hypothesis-panel__inner");
			var $slide = $panel.find(".cro-ai-hypothesis-panel__slide");
