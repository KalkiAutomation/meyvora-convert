...
(function ($) {
	...
	function readBootstrap() {
		var el = document.getElementById('cro-analytics-bootstrap');
	...
	function initMainCharts(b) {
		...
		var mainEl = document.getElementById('cro-main-chart');
		if (mainEl && typeof Chart !== 'undefined' && dailyData && dailyData.length) {
			...
			var mainChart = new Chart(mainCtx, {
				...
				options: Object.assign({}, cro_chart_defaults, {
			...
			document.querySelectorAll('.cro-chart-toggle button').forEach(function (btn) {
				btn.addEventListener('click', function () {
					document.querySelectorAll('.cro-chart-toggle button').forEach(function (x) {
		...
		var devEl = document.getElementById('cro-device-chart');
	...
	function readDateRangeForAjax() {
		var f = document.querySelector('.cro-date-form input[name="from"]');
		var t = document.querySelector('.cro-date-form input[name="to"]');
		var sel = document.querySelector('#cro-campaign-filter');
		return {
			from: f ? f.value : '',
			to: t ? t.value : '',
	...
	function bindAttributionModel(revenueChartState) {
		var radios = document.querySelectorAll('input[name="cro_attribution"]');
		if (!radios.length || typeof croAnalyticsPage === 'undefined' || !croAnalyticsPage.ajaxUrl) {
			return;
		}
		var setNonce = croAnalyticsPage.setAttributionNonce || '';
		var revNonce = croAnalyticsPage.getRevenueNonce || '';
		radios.forEach(function (radio) {
			radio.addEventListener('change', function () {
				...
				$.ajax({
					url: croAnalyticsPage.ajaxUrl,
					type: 'POST',
					dataType: 'json',
					data: {
						action: 'cro_set_attribution_model',
						_wpnonce: setNonce,
						model: model,
					},
				}).done(function (res) {
					...
					$.ajax({
						...
						dataType: 'json',
						data: {
							action: 'cro_get_attributed_revenue',
							_wpnonce: revNonce,
							from: dr.from,
							to: dr.to,
						...
						var valEl = document.querySelector('.js-cro-revenue-kpi-value');
						if (valEl && revHtml) {
							valEl.innerHTML = revHtml;
						}
						var chEl = document.querySelector('.js-cro-revenue-kpi-change');
						if (chEl) {
							chEl.innerHTML = d.change_html || '';
						}
						var rpvEl = document.querySelector('.js-cro-rpv-stat-value');
						if (rpvEl && d.rpv_html) {
							rpvEl.innerHTML = d.rpv_html;
						}
						var tip = document.querySelector('.js-cro-revenue-kpi-tip');
						if (tip && d.tooltip) {
							tip.setAttribute('title', d.tooltip);
						}
						var lbl = document.querySelector('.cro-attribution-model-label');
						...
						$.ajax({
							...
							dataType: 'json',
							data: {
								action: 'cro_get_campaign_revenue_chart',
								_wpnonce: revNonce,
								from: dr.from,
								to: dr.to,
								campaign_id: dr.campaign_id,
							...
							var tip2 = document.querySelector('.js-cro-revenue-kpi-tip');
							if (tip2 && cd.tooltip) {
	...
	function initRevenueChart(b) {
		var canvas = document.getElementById('cro-revenue-attribution-chart');
		...
		document.querySelectorAll('.js-cro-revenue-toggle').forEach(function (btn) {
			btn.addEventListener('click', function () {
				document.querySelectorAll('.js-cro-revenue-toggle').forEach(function (x) {
					x.classList.remove('active');
				});
				btn.classList.add('active');
	...
	function bindAbExpand() {
		document.querySelectorAll('.js-cro-ab-summary-row').forEach(function (row) {
			row.addEventListener('click', function (e) {
				if (e.target.closest('a')) {
					return;
				}
				var id = row.getAttribute('data-test-id');
				var sub = document.querySelector('.js-cro-ab-expand[data-test-id="' + id + '"]');
				if (sub) {
	...
	function bindCopyUrl() {
		document.querySelectorAll('.js-cro-copy-url').forEach(function (btn) {
			btn.addEventListener('click', function (ev) {
				...
				navigator.clipboard.writeText(url).then(function () {
					...
					btn.textContent =
						(window.croAdmin && croAdmin.strings && croAdmin.strings.copied) || 'Copied!';
					setTimeout(function () {
	...
	function bindDatePresets() {
		document.querySelectorAll('.cro-date-presets button').forEach(function (btn) {
			btn.addEventListener('click', function () {
				...
				var form = document.querySelector('.cro-date-form');
				if (form) {
					form.submit();
				}
			});
	...
	$(function () {
		...
		bindTableSort(document.querySelector('.js-cro-top-pages-table'));
		bindAbExpand();
		bindCopyUrl();
		bindDatePresets();
	});
