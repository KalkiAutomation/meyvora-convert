...
$sequences = class_exists( 'CRO_Sequence_Engine' ) ? CRO_Sequence_Engine::get_all() : array();
$campaigns = class_exists( 'CRO_Campaign' ) ? CRO_Campaign::get_all() : array();
...
$edit_id = CRO_Security::get_query_var_absint( 'edit' );
$editing = null;
if ( $edit_id > 0 ) {
...
<div class="cro-sequences-admin">
	...
	<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="cro-sequences-form">
		<?php wp_nonce_field( 'cro_save_sequence', 'cro_sequence_nonce' ); ?>
		<input type="hidden" name="action" value="cro_save_sequence" />
		...
		<table class="widefat striped" id="cro-sequence-steps" style="max-width:720px;">
			<thead>
				<tr>
			...
			<tbody>
				<?php foreach ( $form_steps as $idx => $step ) : ?>
					<tr class="cro-sequence-step-row">
						<td class="cro-sequence-drag-handle" style="cursor:grab;color:var(--cro-text-muted, #646970);padding:0 8px;font-size:18px;user-select:none;" title="<?php esc_attr_e( 'Drag to reorder', 'meyvora-convert' ); ?>">&#8597;</td>
						<td><span class="cro-sequence-step-num"><?php echo esc_html( (string) ( $idx + 1 ) ); ?></span></td>
						<td>
						...
						<td><button type="button" class="button cro-sequence-remove-step" <?php echo count( $form_steps ) <= 1 ? 'style="visibility:hidden"' : ''; ?> aria-label="<?php esc_attr_e( 'Remove step', 'meyvora-convert' ); ?>"><?php esc_html_e( 'Remove', 'meyvora-convert' ); ?></button></td>
					</tr>
				<?php endforeach; ?>
			</tbody>
		</table>
		<p><button type="button" class="button" id="cro-sequence-add-step"><?php esc_html_e( 'Add step', 'meyvora-convert' ); ?></button></p>

		<p class="submit">
			<button type="submit" class="button button-primary"><?php esc_html_e( 'Save sequence', 'meyvora-convert' ); ?></button>
			<?php if ( $editing ) : ?>
				<a class="button" href="<?php echo esc_url( admin_url( 'admin.php?page=cro-sequences' ) ); ?>"><?php esc_html_e( 'Cancel', 'meyvora-convert' ); ?></a>
			<?php endif; ?>
		</p>
	</form>
			...
			<tbody>
				<?php foreach ( $sequences as $row ) : ?>
					...
					<tr>
						...
						<td>
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=cro-sequences&edit=' . $sid ) ); ?>"><?php esc_html_e( 'Edit', 'meyvora-convert' ); ?></a>
							<?php
							$del = wp_nonce_url(
								admin_url( 'admin-post.php?action=cro_delete_sequence&sequence_id=' . $sid ),
								'cro_delete_sequence_' . $sid
							);
							?>
...
<?php ob_start(); ?>
<tr class="cro-sequence-step-row cro-sequence-step-template cro-hidden" hidden>
	<td class="cro-sequence-drag-handle" style="cursor:grab;color:var(--cro-text-muted, #646970);padding:0 8px;font-size:18px;user-select:none;" title="<?php esc_attr_e( 'Drag to reorder', 'meyvora-convert' ); ?>">&#8597;</td>
	<td><span class="cro-sequence-step-num">1</span></td>
	<td>
	...
	<td><button type="button" class="button cro-sequence-remove-step" aria-label="<?php esc_attr_e( 'Remove step', 'meyvora-convert' ); ?>"><?php esc_html_e( 'Remove', 'meyvora-convert' ); ?></button></td>
</tr>
<?php
...
<table class="cro-hidden" aria-hidden="true" style="display:none;"><tbody id="cro-sequence-step-template-wrap"><?php echo wp_kses( $template_html, $allowed_seq_step ); ?></tbody></table>
