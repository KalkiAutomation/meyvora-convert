...
$meyvc_webhook_endpoints = class_exists( 'MEYVC_Webhook' ) ? MEYVC_Webhook::get_endpoints() : array();
$meyvc_webhook_logs      = class_exists( 'MEYVC_Webhook' ) ? MEYVC_Webhook::get_delivery_logs() : array();
$meyvc_webhook_events    = class_exists( 'MEYVC_Webhook' ) ? MEYVC_Webhook::event_names() : array();
$meyvc_webhook_labels    = array(
...
<!-- Webhooks -->
<?php if ( class_exists( 'MEYVC_Webhook' ) ) : ?>
<div class="meyvc-card meyvc-developer-section">
	<header class="meyvc-card__header meyvc-developer-section__header">
		...
		<h2 class="meyvc-card__title"><?php esc_html_e( 'Webhooks', 'meyvora-convert' ); ?></h2>
	</header>
	<div class="meyvc-card__body">
		<p class="meyvc-section-desc"><?php esc_html_e( 'Send signed JSON payloads to your own endpoints when key events occur. Payloads use the meyvora.* event namespace and never include raw email addresses.', 'meyvora-convert' ); ?></p>
		<p>
			<button type="button" class="button button-primary js-meyvc-webhook-toggle-add"><?php esc_html_e( 'Add endpoint', 'meyvora-convert' ); ?></button>
		</p>

		<div id="meyvc-webhook-form-panel" class="meyvc-webhook-form-panel">
			<form id="meyvc-webhook-endpoint-form">
				...
				<p>
					...
					<button type="button" class="button js-meyvc-webhook-generate-secret"><?php esc_html_e( 'Generate', 'meyvora-convert' ); ?></button>
				...
				<fieldset>
					...
					<div class="meyvc-webhook-events-grid">
						<?php foreach ( $meyvc_webhook_events as $ev ) : ?>
						<label>
							<input type="checkbox" name="events[]" value="<?php echo esc_attr( $ev ); ?>" />
							<?php echo esc_html( $meyvc_webhook_labels[ $ev ] ?? $ev ); ?>
						</label>
						<?php endforeach; ?>
						<?php foreach ( $meyvc_webhook_wildcards as $w => $wlabel ) : ?>
						<label>
		...
		<table class="widefat striped meyvc-webhook-endpoints-table" style="margin-top:16px;">
			<thead>
				<tr>
			...
			<tbody>
				<?php if ( empty( $meyvc_webhook_endpoints ) ) : ?>
				<tr><td colspan="5"><?php esc_html_e( 'No webhook endpoints yet.', 'meyvora-convert' ); ?></td></tr>
				<?php else : ?>
					<?php foreach ( $meyvc_webhook_endpoints as $ep ) : ?>
						<?php
						...
						$logs   = ( $eid !== '' && isset( $meyvc_webhook_logs[ $eid ] ) && is_array( $meyvc_webhook_logs[ $eid ] ) ) ? $meyvc_webhook_logs[ $eid ] : array();
					...
					>
						<td><code><?php echo esc_html( MEYVC_Webhook::mask_url( $eurl ) ); ?></code></td>
						<td><?php echo esc_html( (string) count( $evs ) ); ?></td>
						<td><label><input type="checkbox" class="js-meyvc-webhook-active" <?php checked( $active ); ?> /></label></td>
						<td>
							<button type="button" class="button button-small js-meyvc-webhook-test"><?php esc_html_e( 'Test', 'meyvora-convert' ); ?></button>
							<button type="button" class="button button-small js-meyvc-webhook-edit"><?php esc_html_e( 'Edit', 'meyvora-convert' ); ?></button>
							<button type="button" class="button button-small js-meyvc-webhook-delete"><?php esc_html_e( 'Delete', 'meyvora-convert' ); ?></button>
						</td>
						<td>
							<?php if ( empty( $logs10 ) ) : ?>
								<span class="meyvc-muted">—</span>
							<?php else : ?>
								<button type="button" class="button-link js-meyvc-webhook-toggle-logs"><?php esc_html_e( 'Show / hide', 'meyvora-convert' ); ?></button>
								<a href="#" class="js-meyvc-webhook-view-all-logs" data-id="<?php echo esc_attr( $eid ); ?>"><?php esc_html_e( 'View all logs', 'meyvora-convert' ); ?></a>
								<div class="meyvc-webhook-log-expand">
									<table class="widefat" style="margin-top:8px;">
										...
										<?php foreach ( $logs10 as $log ) : ?>
											...
											?>
											<tr class="<?php echo $ok ? 'meyvc-webhook-log--ok' : 'meyvc-webhook-log--err'; ?>">
...
<div id="meyvc-webhook-logs-modal" class="meyvc-webhook-logs-modal" hidden role="dialog" aria-modal="true" aria-labelledby="meyvc-webhook-logs-modal-title">
	<div class="meyvc-webhook-logs-modal__box">
		<div class="meyvc-webhook-logs-modal__head">
			<h3 id="meyvc-webhook-logs-modal-title"><?php esc_html_e( 'Delivery log (last 50)', 'meyvora-convert' ); ?></h3>
			<button type="button" class="button js-meyvc-webhook-logs-modal-close"><?php esc_html_e( 'Close', 'meyvora-convert' ); ?></button>
		</div>
		<div class="meyvc-webhook-logs-modal__body"></div>
	</div>
</div>
<?php endif; ?>
...
<div class="meyvc-card meyvc-developer-section">
		<header class="meyvc-card__header meyvc-developer-section__header">
			...
			<h2 class="meyvc-card__title"><?php esc_html_e( 'Template overrides', 'meyvora-convert' ); ?></h2>
		</header>
		<div class="meyvc-card__body">
			<p class="meyvc-section-desc"><?php esc_html_e( 'Campaign popup templates can be overridden by copying them into your theme.', 'meyvora-convert' ); ?></p>
			<div class="meyvc-developer-code-block meyvc-mb-2">
			...
			<p class="meyvc-mt-2">
				...
				<code>meyvc_popup_template</code> — <?php esc_html_e( 'Change the template file path programmatically. Params: $template_path, $template_type.', 'meyvora-convert' ); ?>
			</p>
		</div>
	</div>
	...
	<div class="meyvc-card meyvc-developer-section">
		<header class="meyvc-card__header meyvc-developer-section__header">
			<span class="meyvc-section-icon"><?php echo wp_kses( MEYVC_Icons::svg( 'zap', array( 'class' => 'meyvc-ico' ) ), MEYVC_Icons::get_svg_kses_allowed() ); ?></span>

			<h2 class="meyvc-card__title"><?php esc_html_e( 'Actions', 'meyvora-convert' ); ?></h2>
		</header>
		<div class="meyvc-card__body">
			<p class="meyvc-section-desc"><?php esc_html_e( 'Fire your code at key points. Use add_action( \'hook_name\', $callback, $priority, $accepted_args ).', 'meyvora-convert' ); ?></p>
			<div class="meyvc-developer-hooks-list">
				<?php foreach ( $actions as $hook_name => $info ) : ?>
					...
					?>
					<div class="meyvc-developer-hook-item">
						<code class="meyvc-developer-hook-name"><?php echo esc_html( $hook_name ); ?></code>
						<?php
						if ( ! empty( $params ) ) {
							...
							?>
							<span class="meyvc-developer-hook-params">( <?php echo esc_html( implode( ', ', $param_names ) ); ?> )</span>
						<?php } ?>
						<?php if ( $desc !== '' ) : ?>
							<p class="meyvc-developer-hook-desc"><?php echo esc_html( $desc ); ?></p>
						<?php endif; ?>
						<?php if ( $example !== '' ) : ?>
							<pre class="meyvc-developer-snippet"><code><?php echo esc_html( $example ); ?></code></pre>
						<?php endif; ?>
					</div>
				<?php endforeach; ?>
			</div>
	...
	<div class="meyvc-card meyvc-developer-section">
		<header class="meyvc-card__header meyvc-developer-section__header">
			...
			<h2 class="meyvc-card__title"><?php esc_html_e( 'Filters', 'meyvora-convert' ); ?></h2>
		</header>
		<div class="meyvc-card__body">
			<p class="meyvc-section-desc"><?php esc_html_e( 'Modify data before it is used. Use add_filter( \'hook_name\', $callback, $priority, $accepted_args ). Return the modified value.', 'meyvora-convert' ); ?></p>
			<div class="meyvc-developer-hooks-list">
				<?php foreach ( $filters as $hook_name => $info ) : ?>
					...
					<div class="meyvc-developer-hook-item">
						...
						<?php } ?>
						<?php if ( $return !== '' ) : ?>
							<span class="meyvc-developer-hook-return">→ <?php echo esc_html( $return ); ?></span>
						<?php endif; ?>
						<?php if ( $desc !== '' ) : ?>
							<p class="meyvc-developer-hook-desc"><?php echo esc_html( $desc ); ?></p>
						<?php endif; ?>
	...
	<div class="meyvc-card meyvc-developer-section">
		<header class="meyvc-card__header meyvc-developer-section__header">
			<span class="meyvc-section-icon"><?php echo wp_kses( MEYVC_Icons::svg( 'edit', array( 'class' => 'meyvc-ico' ) ), MEYVC_Icons::get_svg_kses_allowed() ); ?></span>

			<h2 class="meyvc-card__title"><?php esc_html_e( 'Example snippets', 'meyvora-convert' ); ?></h2>
		</header>
		<div class="meyvc-card__body">
			<div class="meyvc-developer-snippets-grid">
				<div class="meyvc-developer-snippet-card">
					<h4><?php esc_html_e( 'Add custom admin tab', 'meyvora-convert' ); ?></h4>
					<pre class="meyvc-developer-snippet"><code>add_filter( 'meyvc_admin_tabs', function( $tabs ) {
	$tabs['my-tab'] = array(
		'label' => 'My Tab',
	...
	return $tabs;
				...
				<div class="meyvc-developer-snippet-card">
					...
					<pre class="meyvc-developer-snippet"><code>add_action( 'meyvc_campaign_shown', function( $campaign_id, $visitor_id ) {
	// Send to analytics
	gtag( 'event', 'meyvc_popup_shown', array( 'campaign_id' => $campaign_id ) );
}, 10, 2);</code></pre>
				</div>
				<div class="meyvc-developer-snippet-card">
					<h4><?php esc_html_e( 'Modify offer context', 'meyvora-convert' ); ?></h4>
					<pre class="meyvc-developer-snippet"><code>add_filter( 'meyvc_offer_context', function( $context ) {
	...
	return $context;
				...
				<div class="meyvc-developer-snippet-card">
					...
					<pre class="meyvc-developer-snippet"><code>add_filter( 'meyvc_popup_template', function( $path, $template_type ) {
