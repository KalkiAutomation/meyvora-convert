...
/**
 * Class MEYVC_Abandoned_Cart_Reminder
 */
class MEYVC_Abandoned_Cart_Reminder {

	const HOOK  = 'meyvc_abandoned_cart_reminder';
	const GROUP = 'meyvc_abandoned_cart';
	const MAX_EMAILS = 3;
	...
	public function __construct() {
		...
		add_action( 'meyvc_abandoned_cart_process_due_reminders', array( __CLASS__, 'process_due_reminders' ) );
	...
	public static function add_cron_interval( $schedules ) {
		if ( isset( $schedules['meyvc_every_fifteen_minutes'] ) ) {
			return $schedules;
		}
		$schedules['meyvc_every_fifteen_minutes'] = array(
			'interval' => 15 * 60,
	...
	public static function maybe_schedule_recurring_cron() {
		...
		if ( wp_next_scheduled( 'meyvc_abandoned_cart_process_due_reminders' ) ) {
			return;
		}
		wp_schedule_event( time(), 'meyvc_every_fifteen_minutes', 'meyvc_abandoned_cart_process_due_reminders' );
	}
	...
	public static function send_reminder_callback( $abandoned_cart_id, $email_number ) {
		...
		$opts = function_exists( 'meyvc_settings' ) ? meyvc_settings()->get_abandoned_cart_settings() : array();
		$generate_for = isset( $opts['generate_coupon_for_email'] ) ? max( 1, min( 3, (int) $opts['generate_coupon_for_email'] ) ) : 1;
		$coupon_code  = null;
		if ( ! empty( $opts['enable_discount_in_emails'] ) && class_exists( 'MEYVC_Abandoned_Cart_Coupon' ) ) {
			if ( (int) $email_number === (int) $generate_for ) {
				$coupon_code = MEYVC_Abandoned_Cart_Coupon::get_or_create_coupon( $row, $opts );
			} elseif ( ! empty( $row->discount_coupon ) && MEYVC_Abandoned_Cart_Coupon::is_coupon_usable( $row->discount_coupon ) ) {
				$coupon_code = trim( (string) $row->discount_coupon );
			}
		}
		$sent = self::send_email( $row, $email_number, $coupon_code );
		global $wpdb;
		$table = MEYVC_Abandoned_Cart_Tracker::get_table_name();
		if ( $sent ) {
			...
			if ( function_exists( 'do_action' ) ) {
				do_action( 'meyvc_abandoned_cart_email_sent', $abandoned_cart_id, $email_number );
			}
		} else {
	...
	public static function send_reminder_immediately( $cart_id, $email_number = 1 ) {
		...
		$row = class_exists( 'MEYVC_Abandoned_Cart_Tracker' ) ? MEYVC_Abandoned_Cart_Tracker::get_row_by_id( $cart_id ) : null;
		if ( ! $row ) {
			return false;
		}
		if ( ! self::row_can_receive_reminder( $row ) ) {
		...
		if ( $sent ) {
			...
			if ( function_exists( 'do_action' ) ) {
				do_action( 'meyvc_abandoned_cart_email_sent', $cart_id, $email_number );
			}
		}
		return $sent;
	}
	...
	public static function process_due_reminders() {
		...
		$lock_key = 'meyvc_reminder_cron_lock';
		...
		$active = MEYVC_Abandoned_Cart_Tracker::STATUS_ACTIVE;
		$ts_now = time();

		// Broad fetch then per-row segment delays (min high-value delay: 0.5h → 30 min; email 2 min 4h; email 3 min 24h).
		$cache_key_due1 = 'meyvora_meyvc_' . md5( serialize( array( 'abandoned_due_reminders_email_one', $table, $active, $now ) ) );
		$due1           = wp_cache_get( $cache_key_due1, 'meyvora_meyvc' );
		if ( false === $due1 ) {
			...
			wp_cache_set( $cache_key_due1, $due1, 'meyvora_meyvc', 300 );
		} else {
		...
		$cache_key_due2 = 'meyvora_meyvc_' . md5( serialize( array( 'abandoned_due_reminders_email_two', $table, $active, $now ) ) );
		$due2           = wp_cache_get( $cache_key_due2, 'meyvora_meyvc' );
		if ( false === $due2 ) {
			...
			wp_cache_set( $cache_key_due2, $due2, 'meyvora_meyvc', 300 );
		} else {
		...
		$cache_key_due3 = 'meyvora_meyvc_' . md5( serialize( array( 'abandoned_due_reminders_email_three', $table, $active, $now ) ) );
		$due3           = wp_cache_get( $cache_key_due3, 'meyvora_meyvc' );
		if ( false === $due3 ) {
			...
			wp_cache_set( $cache_key_due3, $due3, 'meyvora_meyvc', 300 );
		} else {
	...
	private static function get_segment( array $cart_row ) {
		...
		$threshold = 100.0;
		if ( function_exists( 'meyvc_settings' ) ) {
			$threshold = (float) meyvc_settings()->get( 'abandoned_cart', 'high_value_threshold', 100 );
		}
	...
	private static function get_row( $id ) {
		...
		$cache_key = 'meyvora_meyvc_' . md5( serialize( array( 'abandoned_cart_row_reminder', $table, $id ) ) );
	...
	private static function send_email( $row, $email_number, $coupon_code = null ) {
		...
		$body_tpl = isset( $opts['email_body_template'] ) && trim( (string) $opts['email_body_template'] ) !== ''
			...
			: ( function_exists( 'meyvc_settings' ) ? meyvc_settings()->get_abandoned_cart_email_body_default() : '' );

		$unsubscribe_token = wp_hash( $row->email . '|' . $row->id . '|unsubscribe' );
		$unsubscribe_url   = add_query_arg( array(
			'meyvc_action' => 'unsubscribe_cart',
			'cart_id'    => (int) $row->id,
		...
		$use_ai = class_exists( 'MEYVC_AI_Client' )
			&& MEYVC_AI_Client::is_configured()
			&& class_exists( 'MEYVC_AI_Email_Writer' )
			&& function_exists( 'meyvc_settings' )
			&& 'yes' === meyvc_settings()->get( 'ai', 'feature_emails', 'yes' );

		$ai_bundle = null;
		if ( $use_ai ) {
			try {
				$writer = new MEYVC_AI_Email_Writer();
		...
		$email_template = apply_filters(
			'meyvc_abandoned_cart_email_template',
			MEYVC_PLUGIN_DIR . 'templates/emails/abandoned-cart.php'
		);
	...
	private static function log_ai_email_failure( $cart_id, $message ) {
		...
		if ( class_exists( 'MEYVC_Error_Handler' ) && is_callable( array( 'MEYVC_Error_Handler', 'log' ) ) ) {
			MEYVC_Error_Handler::log(
				'AI_EMAIL',
		...
		if ( ! class_exists( 'MEYVC_Abandoned_Cart_Tracker' ) || ! class_exists( 'MEYVC_Database' ) ) {
			return;
		}
		$table = MEYVC_Abandoned_Cart_Tracker::get_table_name();
		if ( ! MEYVC_Database::table_exists( $table ) ) {
