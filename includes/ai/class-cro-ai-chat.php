...
class CRO_AI_Chat {
	...
	public function chat( string $message, array $history ) {
		...
		$response = CRO_AI_Client::request( $system, $api_messages, 1024 );

		if ( is_wp_error( $response ) ) {
			return $response;
		}

		$text = CRO_AI_Client::extract_text( $response );
	...
	private function build_snapshot(): array {
		...
		$top       = CRO_Analytics::get_top_campaigns( 30, 5 );
	...
	public function register_ajax(): void {
		add_action( 'wp_ajax_cro_ai_chat', array( $this, 'ajax_chat' ) );
	}
	...
	public function ajax_chat(): void {
		...
		check_ajax_referer( 'cro_ai_chat', '_wpnonce' );
