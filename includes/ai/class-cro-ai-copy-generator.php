...
/**
 * Class CRO_AI_Copy_Generator
 */
class CRO_AI_Copy_Generator {

	const AJAX_ACTION = 'cro_ai_generate_copy';
	...
	public function ajax_generate_copy(): void {
		...
		if ( ! wp_verify_nonce( $nonce, 'cro_ai_generate_copy' ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid nonce.', 'meyvora-convert' ) ), 403 );
		}

		if ( function_exists( 'cro_settings' ) && 'yes' !== cro_settings()->get( 'ai', 'feature_copy', 'yes' ) ) {
		...
		if ( ! CRO_AI_Rate_Limiter::check( 'copy_generate', 30 ) ) {
			$reset = CRO_AI_Rate_Limiter::get_reset_time( 'copy_generate' );
		...
		CRO_AI_Rate_Limiter::increment( 'copy_generate' );
