...
/**
 * Class CRO_AI_Email_Writer
 */
class CRO_AI_Email_Writer {

	const TRANSIENT_PREFIX = 'cro_ai_email_';
	 ...
	 *
	 * @param int $cart_id      Row id in cro_abandoned_carts.
	...
	public function register_ajax(): void {
		add_action( 'wp_ajax_cro_ai_preview_email', array( $this, 'ajax_preview_email' ) );
		add_action( 'wp_ajax_cro_ai_bust_email_cache', array( $this, 'ajax_bust_email_cache' ) );
	}
	...
	public function ajax_preview_email(): void {
		...
		if ( ! wp_verify_nonce( $nonce, 'cro_ai_preview_email' ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid nonce.', 'meyvora-convert' ) ), 403 );
		}
		if ( function_exists( 'cro_settings' ) && 'yes' !== cro_settings()->get( 'ai', 'feature_emails', 'yes' ) ) {
		...
		if ( ! class_exists( 'CRO_Abandoned_Cart_Tracker' ) || ! CRO_Database::table_exists( CRO_Abandoned_Cart_Tracker::get_table_name() ) ) {
		...
		if ( ! CRO_AI_Rate_Limiter::check( 'abandoned_email_preview', 40 ) ) {
			$reset = CRO_AI_Rate_Limiter::get_reset_time( 'abandoned_email_preview' );
		...
		CRO_AI_Rate_Limiter::increment( 'abandoned_email_preview' );
	...
	public function ajax_bust_email_cache(): void {
		...
		$nonce = isset( $_POST['_wpnonce'] ) ? sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ) : '';
		if ( ! wp_verify_nonce( $nonce, 'cro_ai_bust_email_cache' ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid nonce.', 'meyvora-convert' ) ), 403 );
		}
		$cart_id      = isset( $_POST['cart_id'] ) ? absint( $_POST['cart_id'] ) : 0;
		$email_number = isset( $_POST['email_number'] ) ? absint( $_POST['email_number'] ) : 0;
	...
	private static function coupon_for_prompt( $row, int $email_number, $recovery_coupon ): string {
		...
		if ( ! empty( $row->discount_coupon ) && class_exists( 'CRO_Abandoned_Cart_Coupon' ) && CRO_Abandoned_Cart_Coupon::is_coupon_usable( $row->discount_coupon ) ) {
	 ...
	 *
	 * @param int         $cart_id         cro_abandoned_carts.id.
	 * @param int         $email_number    1, 2, or 3.
	...
	public function generate_email( int $cart_id, int $email_number, $recovery_coupon = null ) {
		...
		$parsed = CRO_AI_Client::parse_json_response( $raw );
