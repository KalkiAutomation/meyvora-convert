...
class CRO_AI_Insights {
	...
	public static function transient_key( int $days ): string {
		return 'cro_ai_insights_' . $days . 'd';
	}
	...
	public function analyse( int $days = 30, bool $force_refresh = false ) {
		...
		if ( function_exists( 'cro_settings' ) && 'yes' !== cro_settings()->get( 'ai', 'feature_insights', 'yes' ) ) {
			return new WP_Error(
				'feature_disabled',
		...
		if ( ! class_exists( 'CRO_AI_Rate_Limiter' ) || ! CRO_AI_Rate_Limiter::cooldown_ok( self::COOLDOWN_ACTION, self::COOLDOWN_SECONDS ) ) {
			$wait = class_exists( 'CRO_AI_Rate_Limiter' ) ? CRO_AI_Rate_Limiter::cooldown_remaining( self::COOLDOWN_ACTION, self::COOLDOWN_SECONDS ) : self::COOLDOWN_SECONDS;
			return new WP_Error(
				'rate_limited',
		...
		CRO_AI_Rate_Limiter::cooldown_set( self::COOLDOWN_ACTION );
	...
	public function register_ajax(): void {
		add_action( 'wp_ajax_cro_ai_analyse', array( $this, 'ajax_analyse' ) );
		add_action( 'wp_ajax_cro_ai_bust_insights_cache', array( $this, 'ajax_bust_insights_cache' ) );
	}
	...
	public function ajax_analyse(): void {
		...
		if ( ! wp_verify_nonce( $nonce, 'cro_ai_analyse' ) ) {
	...
	public function ajax_bust_insights_cache(): void {
		...
		$nonce = isset( $_POST['_wpnonce'] ) ? sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ) : '';
		if ( ! wp_verify_nonce( $nonce, 'cro_ai_bust_insights_cache' ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid nonce.', 'meyvora-convert' ) ), 403 );
		}
		$days = isset( $_POST['days'] ) ? absint( $_POST['days'] ) : 30;
		$this->bust_cache( $days );
		if ( class_exists( 'CRO_Insights' ) ) {
			CRO_Insights::bust_insights_cache();
		}
	 ...
	 *
	 * @param array $summary Raw summary from CRO_Analytics::get_summary().
	 * @return array
	 */
	...
	private function query_top_offers( string $date_from, string $date_to, int $limit ): array {
		global $wpdb;
		$offers = CRO_Database::get_table( 'offers' );
		$logs   = CRO_Database::get_table( 'offer_logs' );
		if ( ! CRO_Database::table_exists( $offers ) || ! CRO_Database::table_exists( $logs ) ) {
			return array();
		}
	...
	private function query_ab_tests_summary(): array {
		global $wpdb;
		$tests       = CRO_Database::get_table( 'ab_tests' );
		$variations  = CRO_Database::get_table( 'ab_variations' );
		$running     = 0;
		$completed   = array();
		if ( ! CRO_Database::table_exists( $tests ) ) {
			return array(
				'running_count'       => 0,
	...
	private function query_abandoned_stats( string $date_from, string $date_to ): array {
		global $wpdb;
		$table = CRO_Database::get_table( 'abandoned_carts' );
	...
	/**
	 * Last 14 days aggregated from cro_daily_stats (all campaigns).
	 *
	...
	private function query_daily_trend_last_14( string $date_to ): array {
		global $wpdb;
		$stats = CRO_Database::get_table( 'daily_stats' );
		if ( ! CRO_Database::table_exists( $stats ) ) {
			return array();
		}
