...
class CRO_AI_Offer_Suggester {
	...
	public function suggest() {
		...
		if ( function_exists( 'cro_settings' ) && 'yes' !== cro_settings()->get( 'ai', 'feature_offers', 'yes' ) ) {
			return new WP_Error(
				'feature_disabled',
	...
	/**
	 * Register wp_ajax_cro_ai_suggest_offer.
	 */
	public function register_ajax(): void {
		add_action( 'wp_ajax_cro_ai_suggest_offer', array( $this, 'ajax_suggest_offer' ) );
	}
	...
	public function ajax_suggest_offer(): void {
		...
		if ( ! wp_verify_nonce( $nonce, 'cro_ai_suggest_offer' ) ) {
	...
	private function get_existing_active_offers(): array {
		...
		for ( $i = 0; $i < self::MAX_OFFERS; $i++ ) {
			...
			$summary     = class_exists( 'CRO_Offer_Presenter' ) ? CRO_Offer_Presenter::summarize_conditions( $o ) : '';
