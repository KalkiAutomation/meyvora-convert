...
class MEYVC_AI_AB_Hypothesis {
	...
	public static function campaign_is_low_converting( int $campaign_id ): bool {
		...
		if ( $campaign_id < 1 || ! class_exists( 'MEYVC_Analytics' ) ) {
			return false;
		}
	...
	public function generate( int $campaign_id ) {
		...
		if ( function_exists( 'meyvc_settings' ) && 'yes' !== meyvc_settings()->get( 'ai', 'feature_ab', 'yes' ) ) {
			return new WP_Error(
				'feature_disabled',
	...
	public function register_ajax(): void {
		add_action( 'wp_ajax_meyvc_ai_ab_hypothesis', array( $this, 'ajax_ab_hypothesis' ) );
	}
	...
	public function ajax_ab_hypothesis(): void {
		...
		if ( ! wp_verify_nonce( $nonce, 'meyvc_ai_ab_hypothesis' ) ) {
