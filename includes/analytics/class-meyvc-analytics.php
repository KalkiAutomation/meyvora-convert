...
class MEYVC_Analytics {
    ...
    public function __construct() {
        global $wpdb;
        $this->events_table = $wpdb->prefix . 'meyvc_events';
        $this->campaigns_table = $wpdb->prefix . 'meyvc_campaigns';
    }
    ...
    public static function get_overview_stats( $days = 30 ) {
        ...
        $cache_key = 'meyvora_meyvc_' . md5( 'overview_stats_' . serialize( array( (int) $days, $date_from, $date_to ) ) );
        ...
        $coupons_key  = 'meyvora_meyvc_' . md5( 'overview_stats_coupons_' . serialize( array( $date_from, $date_to ) ) );
    ...
    public function get_summary( $date_from = null, $date_to = null, $campaign_id = null ) {
        ...
        $cache_key = 'meyvora_meyvc_' . md5( 'get_summary_' . serialize( array( $date_from, $date_to, $campaign_id ) ) );
        $cached    = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
    ...
    private function cache_table_exists( $table_name ) {
        global $wpdb;
        $cache_key = 'meyvora_meyvc_' . md5( 'table_exists_' . serialize( array( $table_name ) ) );
        $cached    = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return (bool) $cached;
        }
    ...
    private function get_summary_internal( $date_from, $date_to, $campaign_id = null ) {
        ...
        if ( $campaign_id ) {
            $k_imp = 'meyvora_meyvc_' . md5( 'summary_imp_' . serialize( array( $date_from, $date_to, $campaign_id ) ) );
            ...
            $k_clk = 'meyvora_meyvc_' . md5( 'summary_clk_' . serialize( array( $date_from, $date_to, $campaign_id ) ) );
            ...
            $k_conv = 'meyvora_meyvc_' . md5( 'summary_conv_' . serialize( array( $date_from, $date_to, $campaign_id ) ) );
            ...
            $k_rev = 'meyvora_meyvc_' . md5( 'summary_rev_' . serialize( array( $date_from, $date_to, $campaign_id ) ) );
            ...
            $k_em = 'meyvora_meyvc_' . md5( 'summary_em_' . serialize( array( $date_from, $date_to, $campaign_id ) ) );
            ...
            $k_imp = 'meyvora_meyvc_' . md5( 'summary_imp_all_' . serialize( array( $date_from, $date_to ) ) );
            $impressions = wp_cache_get( $k_imp, self::CACHE_GROUP );
            if ( false === $impressions ) {
                $impressions = (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Write operation; caching not applicable.
                $wpdb->prepare(
            ...
            $k_clk = 'meyvora_meyvc_' . md5( 'summary_clk_all_' . serialize( array( $date_from, $date_to ) ) );
            $clicks = wp_cache_get( $k_clk, self::CACHE_GROUP );
            if ( false === $clicks ) {
                $clicks = (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Write operation; caching not applicable.
                $wpdb->prepare(
            ...
            $k_conv = 'meyvora_meyvc_' . md5( 'summary_conv_all_' . serialize( array( $date_from, $date_to ) ) );
            $conversions = wp_cache_get( $k_conv, self::CACHE_GROUP );
            if ( false === $conversions ) {
                $conversions = (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Write operation; caching not applicable.
                $wpdb->prepare(
            ...
            $k_rev = 'meyvora_meyvc_' . md5( 'summary_rev_all_' . serialize( array( $date_from, $date_to ) ) );
            $revenue = wp_cache_get( $k_rev, self::CACHE_GROUP );
            if ( false === $revenue ) {
                $revenue = (float) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Write operation; caching not applicable.
                $wpdb->prepare(
            ...
            $k_em = 'meyvora_meyvc_' . md5( 'summary_em_all_' . serialize( array( $date_from, $date_to ) ) );
            $emails = wp_cache_get( $k_em, self::CACHE_GROUP );
            if ( false === $emails ) {
                $emails = (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Write operation; caching not applicable.
                $wpdb->prepare(
        ...
        $k_sticky = 'meyvora_meyvc_' . md5( 'summary_sticky_' . serialize( array( $date_from, $date_to ) ) );
        ...
        $k_ship = 'meyvora_meyvc_' . md5( 'summary_ship_' . serialize( array( $date_from, $date_to ) ) );
        ...
        if ( $campaign_id ) {
            $pk_imp = 'meyvora_meyvc_' . md5( 'summary_prev_imp_' . serialize( array( $prev_from, $prev_to, $campaign_id ) ) );
            ...
            $pk_conv = 'meyvora_meyvc_' . md5( 'summary_prev_conv_' . serialize( array( $prev_from, $prev_to, $campaign_id ) ) );
            ...
            $pk_rev = 'meyvora_meyvc_' . md5( 'summary_prev_rev_' . serialize( array( $prev_from, $prev_to, $campaign_id ) ) );
            ...
            $pk_em = 'meyvora_meyvc_' . md5( 'summary_prev_em_' . serialize( array( $prev_from, $prev_to, $campaign_id ) ) );
            ...
            $pk_imp = 'meyvora_meyvc_' . md5( 'summary_prev_imp_all_' . serialize( array( $prev_from, $prev_to ) ) );
            $prev_impressions = wp_cache_get( $pk_imp, self::CACHE_GROUP );
            if ( false === $prev_impressions ) {
                $prev_impressions = (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Write operation; caching not applicable.
                $wpdb->prepare(
            ...
            $pk_conv = 'meyvora_meyvc_' . md5( 'summary_prev_conv_all_' . serialize( array( $prev_from, $prev_to ) ) );
            $prev_conversions = wp_cache_get( $pk_conv, self::CACHE_GROUP );
            if ( false === $prev_conversions ) {
                $prev_conversions = (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Write operation; caching not applicable.
                $wpdb->prepare(
            ...
            $pk_rev = 'meyvora_meyvc_' . md5( 'summary_prev_rev_all_' . serialize( array( $prev_from, $prev_to ) ) );
            $prev_revenue = wp_cache_get( $pk_rev, self::CACHE_GROUP );
            if ( false === $prev_revenue ) {
                $prev_revenue = (float) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Write operation; caching not applicable.
                $wpdb->prepare(
            ...
            $pk_em = 'meyvora_meyvc_' . md5( 'summary_prev_em_all_' . serialize( array( $prev_from, $prev_to ) ) );
            $prev_emails = wp_cache_get( $pk_em, self::CACHE_GROUP );
            if ( false === $prev_emails ) {
                $prev_emails = (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- Write operation; caching not applicable.
                $wpdb->prepare(
    ...
    public function get_daily_stats( $date_from, $date_to, $campaign_id = null ) {
        ...
        $campaign_id = ( $campaign_id !== null && $campaign_id > 0 ) ? absint( $campaign_id ) : null;
        $cache_key   = 'meyvora_meyvc_' . md5( 'daily_stats_' . serialize( array( $date_from, $date_to, $campaign_id ) ) );
        $cached      = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
        ...
        $raw_key = 'meyvora_meyvc_' . md5( 'daily_stats_raw_' . serialize( array( $date_from, $date_to, $campaign_id ) ) );
    ...
    public function get_campaign_performance($date_from, $date_to, $limit = 10) {
        ...
        $cache_key = 'meyvora_meyvc_' . md5( 'campaign_performance_' . serialize( array( $date_from, $date_to, (int) $limit ) ) );
        $cached    = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
    ...
    public function get_device_stats($date_from, $date_to) {
        ...
        $cache_key = 'meyvora_meyvc_' . md5( 'device_stats_' . serialize( array( $date_from, $date_to ) ) );
        $cached    = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
    ...
    public function get_top_pages( $date_from, $date_to, $limit = 10, $campaign_id = null ) {
        ...
        $campaign_id = ( $campaign_id !== null && $campaign_id > 0 ) ? absint( $campaign_id ) : null;
        $cache_key   = 'meyvora_meyvc_' . md5( 'top_pages_' . serialize( array( $date_from, $date_to, $limit, $campaign_id ) ) );
        $cached      = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
    ...
    public function get_campaigns_list() {
        ...
        $cache_key = 'meyvora_meyvc_' . md5( 'campaigns_list_' . serialize( array( 'v1' ) ) );
        $cached    = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
    ...
    public function walk_export_events_rows( $date_from, $date_to, $campaign_id, $callback, $chunk_size = 1000 ) {
        ...
        while ( true ) {
            $batch_key = 'meyvora_meyvc_' . md5( 'export_events_batch_' . serialize( array( $date_from, $date_to, $campaign_id, $chunk_size, $offset ) ) );
    ...
    public function get_daily_summary_for_export( $date_from, $date_to ) {
        ...
        $cache_key = 'meyvora_meyvc_' . md5( 'daily_summary_export_' . serialize( array( $date_from, $date_to ) ) );
        $cached    = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
        ...
        $offer_logs   = $wpdb->prefix . 'meyvc_offer_logs';
        ...
        if ( $logs_ok ) {
            $has_action_key = 'meyvora_meyvc_' . md5( 'offer_logs_has_action_col_' . serialize( array( $offer_logs ) ) );
    ...
    public static function get_recent_events( $limit = 10 ) {
        ...
        $cache_key = 'meyvora_meyvc_' . md5( 'recent_events_' . serialize( array( $limit ) ) );
        $cached    = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
    ...
    public function get_revenue_by_campaign( $date_from, $date_to, $campaign_id = null ) {
        ...
        $campaign_id = ( $campaign_id !== null && $campaign_id > 0 ) ? absint( $campaign_id ) : null;
        $cache_key   = 'meyvora_meyvc_' . md5( 'revenue_by_campaign_' . serialize( array( $date_from, $date_to, $campaign_id ) ) );
        $cached      = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
    ...
    /**
     * Conversion funnel counts from events (and meyvc_emails for capture step).
     *
     * @param string   $date_from   Y-m-d.
     * @param string   $date_to     Y-m-d.
    ...
    public function get_conversion_funnel( $date_from, $date_to, $campaign_id = null ) {
        ...
        $campaign_id = ( $campaign_id !== null && $campaign_id > 0 ) ? absint( $campaign_id ) : null;
        $cache_key   = 'meyvora_meyvc_' . md5( 'conversion_funnel_' . serialize( array( $date_from, $date_to, $campaign_id ) ) );
        $cached      = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
    ...
    public function get_cohort_recovery( $date_from, $date_to ) {
        ...
        $cache_key = 'meyvora_meyvc_' . md5( 'cohort_recovery_' . serialize( array( $date_from, $date_to ) ) );
        $cached    = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
    ...
    /**
     * Email capture rate: meyvc_emails rows in period / impressions × 100.
     *
     * @param string   $date_from   Y-m-d.
     * @param string   $date_to     Y-m-d.
    ...
    public function get_email_capture_rate( $date_from, $date_to, $campaign_id = null ) {
        ...
        $campaign_id = ( $campaign_id !== null && $campaign_id > 0 ) ? absint( $campaign_id ) : null;
        $cache_key   = 'meyvora_meyvc_' . md5( 'email_capture_rate_' . serialize( array( $date_from, $date_to, $campaign_id ) ) );
        $cached      = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return (float) $cached;
        }
    ...
    /**
     * Offer revenue from orders linked in meyvc_offer_logs (MYV- coupon prefix).
     *
     * @param string $date_from Y-m-d.
     * @param string $date_to   Y-m-d.
    ...
    public function get_offer_revenue_attribution( $date_from, $date_to ) {
        ...
        $cache_key = 'meyvora_meyvc_' . md5( 'offer_revenue_attr_' . serialize( array( $date_from, $date_to, $this->wc_orders_use_hpos() ? 1 : 0 ) ) );
        $cached    = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
    ...
    public function get_ab_test_summary( $date_from, $date_to ) {
        ...
        $cache_key = 'meyvora_meyvc_' . md5( 'ab_test_summary_' . serialize( array( $date_from, $date_to ) ) );
        $cached    = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }

        if ( ! class_exists( 'MEYVC_AB_Test' ) || ! class_exists( 'MEYVC_AB_Statistics' ) ) {
            wp_cache_set( $cache_key, array(), self::CACHE_GROUP, self::CACHE_TTL );
            return array();
        }

        $tests_table = $wpdb->prefix . 'meyvc_ab_tests';
        ...
        $rows = apply_filters( 'meyvc_analytics_ab_test_summary_raw_rows', $rows, $date_from, $date_to );
        if ( ! is_array( $rows ) ) {
            $rows = array();
        }

        $model = new MEYVC_AB_Test();
        $out   = array();
        foreach ( is_array( $rows ) ? $rows : array() as $row ) {
            $tid = (int) ( $row['id'] ?? 0 );
            if ( ! $tid ) {
            ...
            $stats = in_array( $test->status, array( 'running', 'paused', 'completed' ), true )
                ? MEYVC_AB_Statistics::calculate( $test )
                : null;
            ...
            if ( ! MEYVC_AB_Statistics::has_reached_sample_size( $test ) ) {
    ...
    public function get_abandoned_carts_export_rows( $date_from, $date_to, $limit = 2000 ) {
        ...
        $cache_key = 'meyvora_meyvc_' . md5( 'abandoned_carts_export_' . serialize( array( $date_from, $date_to, $limit ) ) );
        $cached    = wp_cache_get( $cache_key, self::CACHE_GROUP );
        if ( false !== $cached ) {
            return $cached;
        }
