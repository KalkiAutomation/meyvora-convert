...
/**
 * MEYVC_Revenue_Tracker class.
 */
class MEYVC_Revenue_Tracker {
	...
	public function attribute_order_revenue( $order_id ) {
		...
		if ( function_exists( 'meyvc_settings' ) && ! meyvc_settings()->get( 'analytics', 'track_revenue', true ) ) {
			return;
		}
		...
		if ( $order->get_meta( '_meyvc_attributed' ) ) {
			return;
		}

		// Get session ID from cookie.
		$session_id = isset( $_COOKIE['meyvc_session_id'] ) ? sanitize_text_field( wp_unslash( $_COOKIE['meyvc_session_id'] ) ) : '';
		...
		$cache_key_conv = 'meyvora_meyvc_' . md5( serialize( array( 'revenue_session_conversions_24h', $events_table, $session_id, $since ) ) );
		$conversions    = wp_cache_get( $cache_key_conv, 'meyvora_meyvc' );
		if ( false === $conversions ) {
			...
			wp_cache_set( $cache_key_conv, $conversions, 'meyvora_meyvc', 300 );
		}
		...
		$track_coupons = ! function_exists( 'meyvc_settings' ) || meyvc_settings()->get( 'analytics', 'track_coupons', true );
		...
		foreach ( $conversions as $conversion ) {
			...
			if ( $attributed && 'campaign' === $conversion->source_type && $conversion->source_id ) {
				...
				if ( class_exists( 'MEYVC_Database' ) ) {
					MEYVC_Database::invalidate_table_cache_after_write( $campaigns_table );
				}
				...
				if ( false !== $ev_updated ) {
					if ( class_exists( 'MEYVC_Database' ) ) {
						MEYVC_Database::invalidate_table_cache_after_write( $events_table );
					}
					self::flush_meyvora_meyvc_read_cache();
				}

				// Mark order as attributed.
				$order->update_meta_data( '_meyvc_attributed', true );
				$order->update_meta_data( '_meyvc_campaign_id', $conversion->source_id );
				$order->save();
	...
	public static function get_revenue_stats( $days = 30 ) {
		...
		$cache_key_sum = 'meyvora_meyvc_' . md5( serialize( array( 'revenue_stats_sum_order_value', $events_table, $date_limit ) ) );
		$total_revenue = wp_cache_get( $cache_key_sum, 'meyvora_meyvc' );
		if ( false === $total_revenue ) {
			...
			wp_cache_set( $cache_key_sum, $total_revenue, 'meyvora_meyvc', 300 );
		}
		$cache_key_cnt = 'meyvora_meyvc_' . md5( serialize( array( 'revenue_stats_distinct_orders', $events_table, $date_limit ) ) );
		$order_count   = wp_cache_get( $cache_key_cnt, 'meyvora_meyvc' );
		if ( false === $order_count ) {
			...
			wp_cache_set( $cache_key_cnt, $order_count, 'meyvora_meyvc', 300 );
		}
