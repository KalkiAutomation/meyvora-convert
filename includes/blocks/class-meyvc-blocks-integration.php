 ...
 */
class MEYVC_Blocks_Integration_WC implements \Automattic\WooCommerce\Blocks\Integrations\IntegrationInterface {
	 ...
	 * @var string
	 */
	const SCRIPT_HANDLE = 'meyvc-blocks-cart-checkout-extension';
	 ...
	 * @var string
	 */
	const STYLE_BOOSTERS_HANDLE = 'meyvc-blocks-boosters';
	 ...
	 * @var string
	 */
	const STYLE_CHECKOUT_HANDLE = 'meyvc-blocks-checkout';
	...
	private function register_script() {
		$build_dir  = MEYVC_PLUGIN_DIR . 'blocks/cart-checkout-extension/build';
		...
		$ver   = isset( $asset['version'] ) ? $asset['version'] : MEYVC_VERSION;

		wp_register_script(
			self::SCRIPT_HANDLE,
			MEYVC_PLUGIN_URL . 'blocks/cart-checkout-extension/build/index.js',
			$deps,
			$ver,
			true
		);
	...
	public function get_script_handles() {
		$script_path = MEYVC_PLUGIN_DIR . 'blocks/cart-checkout-extension/build/index.js';
	...
	public function get_script_data() {
		...
		$free_shipping_threshold = (float) apply_filters( 'meyvc_free_shipping_threshold', 0 );

		$offer_banner_enabled = false;
		$offer_banner_position = 'both';
		$enable_dynamic_offers = true;
		if ( function_exists( 'meyvc_settings' ) && method_exists( meyvc_settings(), 'get_offer_banner_settings' ) ) {
			$ob = meyvc_settings()->get_offer_banner_settings();
		...
		$enable_dynamic_offers = (bool) apply_filters( 'meyvc_blocks_enable_dynamic_offers', $enable_dynamic_offers );

		$banner_cap_max   = 0;
		$banner_cap_cookie = class_exists( 'MEYVC_Visitor_State' ) ? MEYVC_Visitor_State::BANNER_VIEWS_COOKIE : 'meyvc_banner_views';
		if ( function_exists( 'meyvc_settings' ) && method_exists( meyvc_settings(), 'get_banner_frequency_settings' ) ) {
			$bf = meyvc_settings()->get_banner_frequency_settings();
			$banner_cap_max = (int) ( $bf['max_per_24h'] ?? 0 );
		}

		$rest_url = function_exists( 'rest_url' ) ? rest_url( 'meyvc/v1' ) : '';
		$rest_url = (string) $rest_url;

		$debug = false;
		if ( function_exists( 'meyvc_settings' ) ) {
			$debug = (bool) meyvc_settings()->get( 'general', 'blocks_debug_mode', false );
		}
