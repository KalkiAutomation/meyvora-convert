...
/**
 * CRO_Social_Proof class.
 */
class CRO_Social_Proof {
	...
	public function __construct() {
		if ( ! function_exists( 'cro_settings' ) || ! cro_settings()->is_feature_enabled( 'social_proof' ) ) {
			return;
		}
	...
	public function render() {
		...
		$settings = wp_parse_args(
			cro_settings()->get_group( 'social_proof' ),
			array(
				'window_hours'   => 24,
				'min_quantity'   => 1,
		...
		echo '<div class="cro-social-proof"><span class="cro-social-proof__text">' . esc_html( $msg ) . '</span></div>';
	}
	...
	private function maybe_init_viewing_counter() {
		$settings = cro_settings()->get_group( 'social_proof' );
	...
	public function render_viewing_counter() {
		...
		echo '<div class="cro-viewing-counter" id="cro-viewing-counter" style="display:none;" aria-live="polite"></div>';
	}
	...
	public function enqueue_social_proof_frontend_scripts() {
		...
		wp_enqueue_script(
			...
			CRO_PLUGIN_URL . 'public/js/cro-social-proof-toast' . cro_asset_min_suffix() . '.js',
			array(),
			CRO_VERSION,
			true
		);
	...
	public function bust_purchases_cache(): void {
		delete_transient( 'cro_sp_recent_purchases_10' );
	}
	...
	private function get_recent_purchases( int $limit = 10 ): array {
		$cache_key = 'cro_sp_recent_purchases_' . $limit;
		...
		foreach ( $orders as $order ) {
			...
			if ( ! apply_filters( 'cro_social_proof_include_order', true, $order ) ) {
				continue;
			}
	...
	private function get_recent_sales_count( $product_id, $hours ) {
		...
		$cache_key = 'cro_sp_' . $product_id . '_' . $hours;
		$cached    = get_transient( $cache_key );
