...
class MEYVC_Campaign_Display {
	 ...
	 *
	 * @param string $preview_id      Preview ID (e.g. meyvc_xxx).
	...
	public function render_campaigns() {
		...
		// Frontend: do not output popup HTML here. The controller requests /meyvc/v1/decide and
	...
	private function is_preview_request() {
		...
		if ( ! preg_match( '/^meyvc_[a-zA-Z0-9]+$/', $preview_id ) ) {
			return false;
		}
	...
	/**
	 * Ensure MEYVC_Placeholders and MEYVC_Templates are loaded before popup templates are included.
	...
	private function get_template( $type, $campaign ) {
		...
		// Defense in depth: only registered templates with an on-disk file (see MEYVC_Templates::get_available_for_builder()).
		if ( class_exists( 'MEYVC_Templates' ) ) {
			$available = MEYVC_Templates::get_available_for_builder();
		...
		$template_paths = array(
			'popup' => MEYVC_PLUGIN_DIR . 'templates/popups/' . $template_name . '.php',
		);
		...
		if ( isset( $template_paths[ $type ] ) ) {
			...
			$fallback = MEYVC_PLUGIN_DIR . 'templates/popups/centered.php';
