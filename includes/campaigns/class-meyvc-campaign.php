...
class MEYVC_Campaign {
	...
	/**
	 * Clear `meyvc_has_active_campaigns` and MEYVC_Cache campaign lists after CRUD changes.
	 *
	 * @return void
	 */
	private static function bust_campaign_caches() {
		delete_transient( 'meyvc_has_active_campaigns' );
		delete_transient( 'meyvc_tables_ok' );
		if ( class_exists( 'MEYVC_Cache' ) ) {
			MEYVC_Cache::invalidate_campaigns();
		}
	}
	...
	public static function get_all( $args = array() ) {
		...
		$table_name = $wpdb->prefix . 'meyvc_campaigns';
	...
	public static function get( $id ) {
		...
		$cache_key       = 'meyvora_meyvc_' . md5( serialize( array( 'campaign_row_by_id', $table_name, $id ) ) );
		$cached_campaign = wp_cache_get( $cache_key, 'meyvora_meyvc' );
