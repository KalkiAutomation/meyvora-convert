...
/**
 * CRO_Post_Purchase class.
 */
class CRO_Post_Purchase {

	const SESSION_COUPON_KEY = 'cro_post_purchase_coupon';
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_cro_post_purchase_add', array( $this, 'ajax_add_to_cart' ) );
		add_action( 'wp_ajax_nopriv_cro_post_purchase_add', array( $this, 'ajax_add_to_cart' ) );
	...
	public function render_upsell( $order_id ) {
		...
		$nonce = wp_create_nonce( 'cro_post_purchase' );
		...
		$template = apply_filters(
			'cro_post_purchase_upsell_template',
			CRO_PLUGIN_DIR . 'templates/post-purchase/upsell.php'
		);
	...
	public function ajax_add_to_cart() {
		check_ajax_referer( 'cro_post_purchase', 'nonce' );
	...
	private function get_or_create_upsell_coupon( $percent ) {
		...
		$exist = get_option( 'cro_post_purchase_coupon_' . (int) $percent, '' );
		...
		$code = strtoupper( 'CRO-PP-' . wp_generate_password( 6, false ) );
		...
		$coupon->save();
		update_option( 'cro_post_purchase_coupon_' . (int) $percent, $code, false );

		return $code;
	}

	/**
	 * Track as interaction in cro_events (non-campaign source).
	 *
