...
class CRO_Ajax {
	 ...
	 * @return void
	 */
	private static function cro_ajax_flush_read_cache() {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_cro_onboarding_configure', array( $this, 'onboarding_configure' ) );
		add_action( 'wp_ajax_cro_onboarding_save_state', array( $this, 'onboarding_save_state' ) );
		add_action( 'wp_ajax_cro_onboarding_finish', array( $this, 'onboarding_finish' ) );
		add_action( 'wp_ajax_cro_load_ai_panel_data', array( $this, 'load_ai_panel_data' ) );
		add_action( 'wp_ajax_cro_preview_campaign', array( $this, 'preview_campaign' ) );
		add_action( 'wp_ajax_cro_apply_industry_pack', array( $this, 'apply_industry_pack' ) );
		add_action( 'wp_ajax_cro_spin_init', array( $this, 'spin_init' ) );
		add_action( 'wp_ajax_nopriv_cro_spin_init', array( $this, 'spin_init' ) );
		add_action( 'wp_ajax_cro_spin_capture', array( $this, 'spin_capture' ) );
		add_action( 'wp_ajax_nopriv_cro_spin_capture', array( $this, 'spin_capture' ) );
		add_action( 'wp_ajax_cro_live_stats', array( $this, 'live_stats' ) );
	}
	...
	public function get_campaigns() {
		...
		$cache_key = 'meyvora_cro_' . md5( serialize( array( 'ajax_campaigns_dropdown_list', $table ) ) );
		$campaigns = wp_cache_get( $cache_key, 'meyvora_cro' );
		if ( false === $campaigns ) {
			...
			wp_cache_set( $cache_key, $campaigns, 'meyvora_cro', 300 );
		}
	...
	private function save_wheel_slices_from_post( int $campaign_id, string $wheel_slices_raw = '' ): void {
		...
		global $wpdb;
		$campaigns = $wpdb->prefix . 'cro_campaigns';
		...
		if ( false !== $updated ) {
			if ( class_exists( 'CRO_Database' ) ) {
				CRO_Database::invalidate_table_cache_after_write( $campaigns );
			}
			self::cro_ajax_flush_read_cache();
		}
	}
	...
	public function onboarding_save_state() {
		...
		update_option( 'cro_onboarding_wizard_state', wp_json_encode( $clean ), false );
	...
	public function onboarding_configure() {
		...
		if ( class_exists( 'CRO_Onboarding' ) ) {
			CRO_Onboarding::save_profile_option( $goal, $profile );
			$result = CRO_Onboarding::configure( $goal, $profile );
		} else {
	...
	public function load_ai_panel_data() {
		...
		if ( class_exists( 'CRO_Insights' ) ) {
			$key        = 'cro_insights_' . CRO_Insights::DAYS . 'd';
			$from_cache = false !== get_transient( $key );
			$cards      = CRO_Insights::get_insights( CRO_Insights::DAYS );
		...
		$ai_ok = class_exists( 'CRO_AI_Client' ) && CRO_AI_Client::is_configured();
		$usage = '';
		if ( class_exists( 'CRO_AI_Rate_Limiter' ) && get_current_user_id() > 0 ) {
			$rem = CRO_AI_Rate_Limiter::get_remaining( 'copy_generate', 30 );
			$usage = sprintf(
	...
	public function preview_campaign() {
		...
		$html = CRO_Templates::render_campaign_preview_html( $this->sanitize_array_recursive( $data ) );
	...
	public function apply_industry_pack() {
		...
		if ( ! class_exists( 'CRO_Presets' ) ) {
			wp_send_json_error( array( 'message' => __( 'Presets unavailable', 'meyvora-convert' ) ) );
		}
		$result = CRO_Presets::apply_industry_pack( $pack_id );
	...
	public function spin_init(): void {
		...
		if ( class_exists( 'CRO_Security' ) ) {
			$rl_key = 'cro_spin_init_' . md5( CRO_Security::get_client_ip() . '|' . $campaign_id );
			if ( ! CRO_Security::check_rate_limit( $rl_key, 5, 3600 ) ) {
		...
		$seed = crc32( (string) $campaign_id . '|' . ( class_exists( 'CRO_Security' ) ? CRO_Security::get_client_ip() : '' ) . '|' . gmdate( 'YmdH' ) );
	...
	private function get_wheel_slices( int $campaign_id ): array {
		...
		if ( $campaign_id > 0 ) {
			global $wpdb;
			$table     = $wpdb->prefix . 'cro_campaigns';
			$cache_key = 'meyvora_cro_' . md5( serialize( array( 'ajax_wheel_slices_json_column', $table, $campaign_id ) ) );
			$raw       = wp_cache_get( $cache_key, 'meyvora_cro' );
			if ( false === $raw ) {
				...
				wp_cache_set( $cache_key, $raw, 'meyvora_cro', 300 );
			}
			if ( $raw ) {
		...
		return (array) apply_filters( 'cro_wheel_slices', $defaults, $campaign_id );
	}
	...
	public function spin_capture(): void {
		...
		if ( class_exists( 'CRO_Security' ) ) {
			$rl_key = 'cro_spin_cap_' . md5( $email . '|' . CRO_Security::get_client_ip() );
			if ( ! CRO_Security::check_rate_limit( $rl_key, 3, 3600 ) ) {
		...
		do_action( 'cro_email_captured', $email, $campaign_id, array( 'source' => 'spin_wheel', 'slice' => $slice_label, 'is_win' => $is_win ) );

		$response = array( 'captured' => true, 'is_win' => $is_win );

		if ( $is_win && class_exists( 'CRO_Offer_Engine' ) ) {
			$coupon_code = CRO_Offer_Engine::get_or_create_coupon_for_best_offer();
	...
	public function live_stats() {
		...
		$ac_table     = $wpdb->prefix . 'cro_abandoned_carts';
		...
		if ( class_exists( 'CRO_Database' ) && CRO_Database::table_exists( $events_table ) ) {
			$cache_key_live = 'meyvora_cro_' . md5( serialize( array( 'ajax_live_stats_events_window', $events_table, 60 ) ) );
			$cached_live    = wp_cache_get( $cache_key_live, 'meyvora_cro' );
			if ( false === $cached_live ) {
				...
				wp_cache_set( $cache_key_live, $row, 'meyvora_cro', 300 );
			} else {
		...
		if ( class_exists( 'CRO_Database' ) && CRO_Database::table_exists( $ac_table ) ) {
			$cache_key_ac = 'meyvora_cro_' . md5( serialize( array( 'ajax_live_stats_recovered_carts', $ac_table, 'recovered', 60 ) ) );
			$cr_raw         = wp_cache_get( $cache_key_ac, 'meyvora_cro' );
			if ( false === $cr_raw ) {
				...
				wp_cache_set( $cache_key_ac, $cr_raw, 'meyvora_cro', 300 );
			}
