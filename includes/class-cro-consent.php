 ...
 *
 * Auto-detects active consent plugins and wires up the cro_consent_allows_popup filter.
...
class CRO_Consent {
	...
	public static function init(): void {
		...
		add_filter( 'cro_consent_allows_popup', array( __CLASS__, 'check_consent' ), 10, 1 );
	}
	...
	private static function consent_checking_enabled(): bool {
		...
		return (bool) cro_settings()->get( 'general', 'require_cookie_consent', false );
	}
	...
	public static function check_consent( $allow = true ): bool {
		...
		if ( class_exists( '\BorlabsCookie\Cookie\Frontend\Consent' ) ) {
			try {
				...
				do_action( 'cro_borlabs_consent_api_error', $e );
			}
		}
