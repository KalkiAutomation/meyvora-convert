...
/**
 * CRO_Dashboard_Metrics
 */
class CRO_Dashboard_Metrics {
	...
	public static function get_recovered_revenue_30d() {
		...
		$cache_key = 'meyvora_cro_' . md5( serialize( array( 'dashboard_recovered_revenue_30d', $table ) ) );
		$cached    = wp_cache_get( $cache_key, 'meyvora_cro' );
		if ( false !== $cached ) {
			return (float) $cached;
		}
		...
		if ( $wpdb->last_error || null === $sum ) {
			...
			wp_cache_set( $cache_key, $php, 'meyvora_cro', 300 );
			return $php;
		}
		$out = (float) $sum;
		wp_cache_set( $cache_key, $out, 'meyvora_cro', 300 );
		return $out;
	}
	...
	private static function sum_recovered_totals_php( $table ) {
		global $wpdb;
		$cache_key = 'meyvora_cro_' . md5( serialize( array( 'dashboard_recovered_cart_json_rows', $table ) ) );
	...
	public static function get_emails_captured_30d() {
		...
		$cache_key = 'meyvora_cro_' . md5( serialize( array( 'dashboard_emails_captured_30d', $table, 30 ) ) );
	...
	public static function get_offer_conversions_30d() {
		...
		$cache_key = 'meyvora_cro_' . md5( serialize( array( 'dashboard_offer_conversions_30d', $table, 'conversion', 'offer', 30 ) ) );
		$result    = wp_cache_get( $cache_key, 'meyvora_cro' );
		if ( false === $result ) {
			$result = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above.
				$wpdb->prepare(
	...
	public static function get_avg_order_value_90d() {
		...
		$cache_key_exists = 'meyvora_cro_' . md5( serialize( array( 'dashboard_wc_orders_table_exists', $orders ) ) );
		$table_exists     = wp_cache_get( $cache_key_exists, 'meyvora_cro' );
		if ( false === $table_exists ) {
			...
			wp_cache_set( $cache_key_exists, $table_exists, 'meyvora_cro', 300 );
		}
		if ( $table_exists !== $orders ) {
			return 0.0;
		}
		$cache_key_avg = 'meyvora_cro_' . md5( serialize( array( 'dashboard_avg_order_value_90d', $orders, 90 ) ) );
		$avg           = wp_cache_get( $cache_key_avg, 'meyvora_cro' );
		if ( false === $avg ) {
			...
			wp_cache_set( $cache_key_avg, $avg, 'meyvora_cro', 300 );
		}
	...
	public static function get_recent_wins( $limit = 5 ) {
		...
		$ac     = $wpdb->prefix . 'cro_abandoned_carts';
		$events = $wpdb->prefix . 'cro_events';
		$offers = $wpdb->prefix . 'cro_offers';
		$out    = array();

		if ( class_exists( 'CRO_Database' ) && CRO_Database::table_exists( $ac ) ) {
			$cache_key_carts = 'meyvora_cro_' . md5( serialize( array( 'dashboard_recent_recovered_carts', $ac, 'recovered', $limit * 2 ) ) );
			$carts           = wp_cache_get( $cache_key_carts, 'meyvora_cro' );
			if ( false === $carts ) {
				...
				wp_cache_set( $cache_key_carts, $carts, 'meyvora_cro', 300 );
			}
			if ( ! is_array( $carts ) ) {
				$carts = array();
			}
		...
		if ( class_exists( 'CRO_Database' ) && CRO_Database::table_exists( $events ) && CRO_Database::table_exists( $offers ) ) {
			$cache_key_wins = 'meyvora_cro_' . md5( serialize( array( 'dashboard_recent_offer_conversions', $events, $offers, 'conversion', 'offer', $limit * 2 ) ) );
			$rows           = wp_cache_get( $cache_key_wins, 'meyvora_cro' );
			if ( false === $rows ) {
				$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above.
					$wpdb->prepare(
				...
				wp_cache_set( $cache_key_wins, $rows, 'meyvora_cro', 300 );
			}
			if ( ! is_array( $rows ) ) {
				$rows = array();
			}
