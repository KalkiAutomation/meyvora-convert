 ...
 * @return bool
 */
function cro_is_onboarding_complete() {
	if ( get_option( 'cro_onboarding_complete', false ) ) {
		return true;
	}
	return (bool) get_option( 'cro_onboarding_completed', false );
}
 ...
 * @return void
 */
function cro_mark_onboarding_complete() {
...
class CRO_Onboarding {
	...
	public static function configure( $goal, array $profile ) {
		...
		if ( ! $settings || ! class_exists( 'CRO_Campaign' ) ) {
			return array(
				'features'  => array(),
				'campaigns' => array(),
			);
		...
		$result = apply_filters(
			'cro_onboarding_configure_result',
			array(
	...
	public static function save_profile_option( $goal, array $profile ) {
		...
		update_option( 'cro_onboarding_profile', wp_json_encode( $payload ), false );
	}
	...
	private static function create_campaign( $name, $campaign_type, $template, array $content, array $targeting_rules, array $trigger_overrides = array() ) {
		...
		$data = array(
			...
			'styling'          => class_exists( 'CRO_Activator' ) ? CRO_Activator::get_default_styling() : $model->styling,
