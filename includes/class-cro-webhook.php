...
class CRO_Webhook {
	...
	public static function init() {
		...
		add_action( 'cro_abandoned_cart_email_sent', array( __CLASS__, 'on_abandoned_cart_email_sent' ), 20, 2 );
		...
		add_action( 'wp_ajax_cro_save_webhook_endpoint', array( __CLASS__, 'ajax_save_endpoint' ) );
		add_action( 'wp_ajax_cro_delete_webhook_endpoint', array( __CLASS__, 'ajax_delete_endpoint' ) );
		add_action( 'wp_ajax_cro_toggle_webhook_endpoint', array( __CLASS__, 'ajax_toggle_endpoint' ) );
		add_action( 'wp_ajax_cro_test_webhook_endpoint', array( __CLASS__, 'ajax_test_endpoint' ) );
		add_action( 'wp_ajax_cro_get_webhook_logs', array( __CLASS__, 'ajax_get_logs' ) );
	}
	...
	public static function get_endpoints() {
		...
		$raw = cro_settings()->get( self::SETTINGS_GROUP, self::ENDPOINTS_KEY, array() );
		if ( is_string( $raw ) ) {
	...
	public static function save_endpoints( array $endpoints ) {
		if ( function_exists( 'cro_settings' ) ) {
			cro_settings()->set( self::SETTINGS_GROUP, self::ENDPOINTS_KEY, array_values( $endpoints ) );
		}
	}
	...
	public static function get_delivery_logs() {
		...
		$raw = cro_settings()->get( self::SETTINGS_GROUP, self::DELIVERY_LOG_KEY, array() );
		if ( is_string( $raw ) ) {
			$decoded = json_decode( $raw, true );
			$raw     = is_array( $decoded ) ? $decoded : array();
		}
	...
	public static function append_delivery_log( $endpoint_id, $event_name, $status, $ms, $error = '' ) {
		...
		if ( function_exists( 'cro_settings' ) ) {
			cro_settings()->set( self::SETTINGS_GROUP, self::DELIVERY_LOG_KEY, $logs );
		}
	}
	...
	public static function dispatch( $event_name, array $payload ) {
		...
		$payload = apply_filters( 'cro_webhook_inner_payload', $payload, $event_name );
		if ( ! is_array( $payload ) ) {
			$payload = array();
		}
	...
	public static function deliver( $endpoint_id, $event_name, array $inner, $attempt = 0 ) {
		...
		$body_arr = apply_filters( 'cro_webhook_envelope', self::build_envelope( $event_name, $inner ), $event_name, $inner );
	...
	/**
	 * @param array $event cro_event_tracked payload.
	 */
	 ...
	 * @param array  $stats Stats from CRO_AB_Statistics::calculate.
	 */
	...
	public static function on_wc_applied_coupon( $coupon_code ) {
		...
		if ( class_exists( 'CRO_Offer_Model' ) ) {
			$o = CRO_Offer_Model::get( $offer_id );
		...
		if ( class_exists( 'CRO_Visitor_State' ) ) {
			$vs = CRO_Visitor_State::get_instance();
	...
	public static function on_offer_order_placed( $order_id ) {
		...
		$done = $order->get_meta( '_cro_meyvora_offer_conv_codes', true );
		...
		foreach ( $order->get_coupon_codes() as $code ) {
			...
			if ( $offer_id > 0 && class_exists( 'CRO_Offer_Model' ) ) {
				$om = CRO_Offer_Model::get( $offer_id );
		...
		if ( $any ) {
			$order->update_meta_data( '_cro_meyvora_offer_conv_codes', $done );
			$order->save();
		}
	}
	...
	private static function build_offer_converted_payload( array $event ) {
		...
		$name     = '';
		if ( $offer_id && class_exists( 'CRO_Offer_Model' ) ) {
			$o = CRO_Offer_Model::get( $offer_id );
			if ( is_object( $o ) && isset( $o->name ) ) {
				$name = (string) $o->name;
			}
	...
	private static function get_abandoned_row( $id ) {
		...
		return CRO_Abandoned_Cart_Tracker::get_row_by_id( $id );
	}
