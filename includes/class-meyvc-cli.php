...
class MEYVC_CLI_Command {
	 ...
	 * ## EXAMPLES
	 *
	 *     wp meyvc verify-package
	 *     wp meyvc verify-package --format=json
	 *
	...
	public function verify_package( $args, $assoc_args ) {
		if ( ! class_exists( 'MEYVC_System_Status' ) ) {
			WP_CLI::error( 'MEYVC_System_Status not loaded.' );
		}
		$results = MEYVC_System_Status::run_verify_package();
		$all_pass = true;
		$rows = array();
	 ...
	 * ## EXAMPLES
	 *
	 *     wp meyvc test-abandoned-segment --cart-id=12
	 *
	 * @param array $args       Positional args.
	 * @param array $assoc_args Associative args.
	 */
	public function test_abandoned_segment( $args, $assoc_args ) {
		...
		if ( ! class_exists( 'MEYVC_Abandoned_Cart_Tracker' ) || ! class_exists( 'MEYVC_Abandoned_Cart_Reminder' ) ) {
		...
		$debug = MEYVC_Abandoned_Cart_Reminder::get_schedule_debug( $row );
...
WP_CLI::add_command( 'meyvc', 'MEYVC_CLI_Command' );
