...
/**
 * MEYVC_Compatibility class.
 *
 * Detects checkout type, theme, conflicting plugins, and provides overrides.
 */
class MEYVC_Compatibility {

	/**
	 * Singleton instance.
	 *
	 * @var MEYVC_Compatibility|null
	 */
	...
	public function detect_checkout_type() {
		if ( ! function_exists( 'meyvc_settings' ) || ! function_exists( 'wc_get_page_id' ) ) {
			return;
		}
		...
		if ( class_exists( 'Automattic\WooCommerce\Blocks\Package' ) ) {
			...
			if ( $checkout_page && has_block( 'woocommerce/checkout', $checkout_page ) ) {
				meyvc_settings()->set( 'compatibility', 'checkout_type', 'blocks' );
				return;
			}
		}

		meyvc_settings()->set( 'compatibility', 'checkout_type', 'classic' );
	}
	...
	public function detect_cart_type() {
		...
		if ( class_exists( 'Automattic\WooCommerce\Blocks\Package' ) ) {
			...
			if ( $cart_page && has_block( 'woocommerce/cart', $cart_page ) ) {
				meyvc_settings()->set( 'compatibility', 'cart_type', 'blocks' );
				return;
			}
		}

		meyvc_settings()->set( 'compatibility', 'cart_type', 'classic' );
	}
	...
	public function get_cart_type() {
		return function_exists( 'meyvc_settings' ) ? meyvc_settings()->get( 'compatibility', 'cart_type', 'classic' ) : 'classic';
	}
	...
	public function get_checkout_type() {
		return function_exists( 'meyvc_settings' ) ? meyvc_settings()->get( 'compatibility', 'checkout_type', 'classic' ) : 'classic';
	}
	...
	public function detect_theme() {
		...
		if ( isset( $special_themes[ $theme_slug ] ) ) {
			meyvc_settings()->set( 'compatibility', 'theme_overrides', $special_themes[ $theme_slug ] );
		} else {
			meyvc_settings()->set( 'compatibility', 'theme_overrides', array() );
		}
	...
	public function detect_conflicting_plugins() {
		...
		if ( ! function_exists( 'is_plugin_active' ) ) {
			if ( is_admin() && file_exists( ABSPATH . 'wp-admin/includes/plugin.php' ) ) {
				...
				if ( function_exists( 'meyvc_settings' ) ) {
					meyvc_settings()->set( 'compatibility', 'conflicts', $conflicts );
				}
				return;
			}
		}
		...
		if ( class_exists( 'WC_Subscriptions' ) || is_plugin_active( 'woocommerce-subscriptions/woocommerce-subscriptions.php' ) ) {
			if ( function_exists( 'meyvc_settings' ) ) {
				meyvc_settings()->set( 'compatibility', 'wc_subscriptions_active', true );
			}
		}
	...
	public function show_compatibility_notices() {
		...
		if ( ! $screen || strpos( (string) ( $screen->id ?? '' ), 'meyvc-' ) === false ) {
			return;
		}
		...
		$conflicts = meyvc_settings()->get( 'compatibility', 'conflicts', array() );
...
add_action( 'plugins_loaded', function() {
	if ( function_exists( 'meyvc_settings' ) ) {
		MEYVC_Compatibility::get_instance();
	}
}, 25 );
