 ...
 */
function meyvc_quick_launch_apply( $preset ) {
	if ( $preset !== 'recommended' ) {
		return false;
	}

	if ( ! function_exists( 'meyvc_settings' ) || ! class_exists( 'MEYVC_Database' ) ) {
		return false;
	}
	...
	MEYVC_Database::begin_transaction();
	...
	if ( $ok ) {
		MEYVC_Database::commit();
	} else {
		MEYVC_Database::rollback();
	}
	return $ok;
}
