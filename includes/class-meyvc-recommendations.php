...
/**
 * MEYVC_Recommendations class.
 */
class MEYVC_Recommendations {
	...
	public function __construct() {
		if ( ! function_exists( 'meyvc_settings' ) || ! meyvc_settings()->is_feature_enabled( 'recommendations' ) ) {
			return;
		}
	...
	public function render_frequently_bought(): void {
		...
		echo '<section class="meyvc-recommendations meyvc-frequently-bought">';
		echo '<h3 class="meyvc-recommendations__title">' . esc_html__( 'Frequently bought together', 'meyvora-convert' ) . '</h3>';
		echo '<ul class="meyvc-recommendations__list">';
		foreach ( $recs as $pid ) {
			$p = wc_get_product( $pid );
			if ( ! $p || ! $p->is_purchasable() || ! $p->is_visible() ) {
				continue;
			}
			echo '<li class="meyvc-recommendations__item">';
			echo '<a href="' . esc_url( get_permalink( $pid ) ) . '">' . wp_kses_post( $p->get_image( 'woocommerce_thumbnail' ) ) . '</a>';
			echo '<a class="meyvc-recommendations__name" href="' . esc_url( get_permalink( $pid ) ) . '">' . esc_html( $p->get_name() ) . '</a>';
			echo '<span class="meyvc-recommendations__price">' . wp_kses_post( $p->get_price_html() ) . '</span>';
			echo '</li>';
		}
		echo '</ul>';
		echo '</section>';
	...
	public function render_cart_recommendations(): void {
		...
		echo '<div class="meyvc-recommendations meyvc-cart-recommendations"><h4>' . esc_html__( 'You might also like', 'meyvora-convert' ) . '</h4><ul class="meyvc-recommendations__list">';
	...
	public function get_co_purchased( int $product_id, int $limit = 4 ): array {
		$cache_key = 'meyvc_rec_' . $product_id . '_' . $limit;
