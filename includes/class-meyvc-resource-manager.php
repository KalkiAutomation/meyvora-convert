...
class MEYVC_Resource_Manager {
	...
	public static function free_memory() {
		if ( class_exists( 'MEYVC_Cache' ) && method_exists( 'MEYVC_Cache', 'flush' ) ) {
			MEYVC_Cache::flush();
		} else {
			wp_cache_flush();
		}
		...
		foreach ( $chunks as $chunk ) {
			if ( self::is_memory_critical() || self::is_time_critical() ) {
				if ( class_exists( 'MEYVC_Error_Handler' ) ) {
					MEYVC_Error_Handler::log( 'WARNING', 'Resource limit reached during iteration' );
				}
				break;
			}
...
add_action( 'init', array( 'MEYVC_Resource_Manager', 'init' ), 1 );
