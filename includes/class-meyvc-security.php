...
class MEYVC_Security {
	 ...
	 * @return true
	 */
	public static function verify_ajax_nonce( $action = 'meyvc_ajax_nonce' ) {
	...
	public static function sanitize_email_input( $email ) {
		...
		$disposable_domains = apply_filters(
			'meyvc_disposable_email_domains',
	...
	public static function get_visitor_ip(): string {
		...
		$anonymise = function_exists( 'meyvc_settings' ) && meyvc_settings()->get( 'analytics', 'anonymise_ip', false );
		if ( $anonymise && $ip !== '' ) {
			if ( strpos( $ip, ':' ) !== false ) {
				$parts = explode( ':', $ip );
	...
	public static function check_rate_limit( $key, $limit = 10, $window = 60 ) {
		...
		$transient_key = 'meyvc_rate_' . md5( $key );
		$group         = 'meyvc_rate_limits';
