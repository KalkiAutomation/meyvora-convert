...
/**
 * MEYVC_Settings class.
 *
 * Manages all plugin settings via the meyvc_settings table.
 */
class MEYVC_Settings {
	...
	private function __construct() {
		global $wpdb;
		$this->table_name = $wpdb->prefix . 'meyvc_settings';
		$this->load_settings();
	}
	...
	private function load_settings() {
		...
		$cache_key = 'meyvora_meyvc_' . md5( serialize( array( 'settings_all_rows', $this->table_name ) ) );
		$results   = wp_cache_get( $cache_key, 'meyvora_meyvc' );
		if ( false === $results ) {
			...
			if ( is_array( $results ) ) {
				wp_cache_set( $cache_key, $results, 'meyvora_meyvc', 300 );
			}

			// Self-heal: if query failed (e.g. missing table), create tables and retry once per request.
			if ( null === $results && $wpdb->last_error && class_exists( 'MEYVC_Database' ) && ! self::$settings_repair_attempted ) {
				self::$settings_repair_attempted = true;
				MEYVC_Database::create_tables();
				// Defer defaults — MEYVC_Activator::set_default_settings() uses MEYVC_Settings::get_instance()
				// and cannot run synchronously while this constructor is still running.
				if ( class_exists( 'MEYVC_Activator' ) && ! self::$repair_shutdown_scheduled ) {
					self::$repair_shutdown_scheduled = true;
					add_action(
						'shutdown',
						static function () {
							if ( class_exists( 'MEYVC_Activator' ) ) {
								MEYVC_Activator::set_default_settings();
							}
							if ( class_exists( 'MEYVC_Settings' ) ) {
								MEYVC_Settings::get_instance()->reload();
							}
						},
						1
					);
	...
	public function delete( $group, $key ) {
		...
		if ( false !== $result ) {
			if ( class_exists( 'MEYVC_Database' ) ) {
				MEYVC_Database::invalidate_table_cache_after_write( $this->table_name );
			}
			self::flush_meyvora_meyvc_read_cache();
 ...
 * @return MEYVC_Settings
 */
function meyvc_settings() {
	return MEYVC_Settings::get_instance();
}
