...
/**
 * MEYVC_System_Status class.
 */
class MEYVC_System_Status {
	...
	public static function run_verify_package() {
		...
		if ( class_exists( 'MEYVC_Database' ) ) {
			foreach ( MEYVC_Database::get_required_table_names() as $name ) {
				$table = MEYVC_Database::get_table( $name );
				if ( ! MEYVC_Database::table_exists( $table ) ) {
					$missing_tables[] = $table;
				}
			}
		} else {
			$missing_tables[] = __( 'MEYVC_Database not loaded', 'meyvora-convert' );
		}
		$results[] = array(
		...
		$build_dir = defined( 'MEYVC_PLUGIN_DIR' ) ? MEYVC_PLUGIN_DIR . 'blocks/cart-checkout-extension/build/' : '';
		...
		if ( function_exists( 'apply_filters' ) ) {
			$forced_global = apply_filters( 'meyvc_should_enqueue_assets', null, 'global' );
		}
		$results[] = array(
	...
	public static function run_verify_installation() {
		...
		$integration_class = class_exists( 'MEYVC_Blocks_Integration_WC' );
		...
		$offer_url = rest_url( 'meyvc/v1/offer' );
		...
		if ( ! empty( $missing_tables ) && class_exists( 'MEYVC_Database' ) ) {
			$delta_output = array();
			$ok = MEYVC_Database::create_tables( $delta_output );
		...
		$builder_css = defined( 'MEYVC_PLUGIN_DIR' ) && is_readable( MEYVC_PLUGIN_DIR . 'admin/css/meyvc-campaign-builder.css' );
		$builder_js  = defined( 'MEYVC_PLUGIN_DIR' ) && is_readable( MEYVC_PLUGIN_DIR . 'admin/js/meyvc-campaign-builder.js' );
		$results[] = array(
			'label'   => __( 'Campaign builder assets exist', 'meyvora-convert' ),
			'pass'    => $builder_css && $builder_js,
			'message' => ( $builder_css && $builder_js )
				? __( 'meyvc-campaign-builder.css and .js present.', 'meyvora-convert' )
				: sprintf(
					/* translators: %s is a comma-separated list of missing asset file names. */
					__( 'Missing: %s', 'meyvora-convert' ),
					implode( ', ', array_filter( array(
						$builder_css ? '' : 'admin/css/meyvc-campaign-builder.css',
						$builder_js ? '' : 'admin/js/meyvc-campaign-builder.js',
					) ) )
				),
		);

		// 10) UI & Builder Health: no legacy CRO admin CSS (design system is source of truth)
		$legacy_meyvc_css = array();
		$meyvc_css_handles = array();
		if ( isset( $GLOBALS['wp_styles'] ) && $GLOBALS['wp_styles'] instanceof \WP_Styles ) {
			...
			$legacy_handles = array( 'meyvc-admin', 'meyvc-admin-ui', 'meyvc-admin-modern', 'meyvc-admin-brand-identity' );
			foreach ( $legacy_handles as $handle ) {
				if ( in_array( $handle, $done, true ) ) {
					$legacy_meyvc_css[] = $handle;
				}
			}
			foreach ( $done as $handle ) {
				if ( strpos( $handle, 'cro' ) !== false && isset( $GLOBALS['wp_styles']->registered[ $handle ] ) ) {
					$meyvc_css_handles[] = $handle;
				}
			}
		}
		$results[] = array(
			'label'   => __( 'UI & Builder: no legacy CRO admin CSS', 'meyvora-convert' ),
			'pass'    => empty( $legacy_meyvc_css ),
			'message' => empty( $legacy_meyvc_css )
				? ( empty( $meyvc_css_handles ) ? __( 'Design system and page-specific only.', 'meyvora-convert' ) : sprintf( /* translators: %s is a comma-separated list of CSS handles. */ __( 'CRO CSS loaded: %s', 'meyvora-convert' ), implode( ', ', $meyvc_css_handles ) ) )
				: sprintf(
				/* translators: %s is a comma-separated list of CSS handle names. */
					__( 'Legacy CSS enqueued (remove from code): %s', 'meyvora-convert' ),
					implode( ', ', $legacy_meyvc_css )
				),
		);

		// 11) Builder script registered (in register_campaign_builder_assets); enqueued on page=meyvc-campaign-edit or meyvc-campaign-builder
		$builder_registered = wp_script_is( 'meyvc-campaign-builder', 'registered' );
		$results[] = array(
			...
			'message' => $builder_registered
				...
				: __( 'meyvc-campaign-builder script not registered. Check register_campaign_builder_assets.', 'meyvora-convert' ),
		);
	...
	private static function check_db_tables() {
		global $wpdb;
		// Table names must match MEYVC_Database::create_tables() (prefix-safe).
		$tables = array(
		...
		if ( empty( $missing ) ) {
			$col_warnings = array();
			$campaigns_tbl = $wpdb->prefix . 'meyvc_campaigns';
			$offers_tbl    = $wpdb->prefix . 'meyvc_offers';
			...
			if ( ! $has_fb ) {
				$col_warnings[] = __( 'meyvc_campaigns.fallback_delay_seconds missing (run Repair Database Tables).', 'meyvora-convert' );
			}
			...
			if ( ! $has_conflict ) {
				$col_warnings[] = __( 'meyvc_offers.conflict_ids missing (run Repair Database Tables).', 'meyvora-convert' );
			}
			if ( ! empty( $col_warnings ) ) {
				return array(
					...
					'detail'  => __( 'Use “Repair Database Tables” on the System Status page to apply migrations (MEYVC_Activator::maybe_upgrade_tables).', 'meyvora-convert' ),
				);
			}
			return array(
				'label'   => __( 'Custom tables status', 'meyvora-convert' ),
	...
	private static function check_ai_configuration() {
		...
		if ( function_exists( 'meyvc_settings' ) ) {
			foreach ( $ai_features as $f ) {
				if ( 'yes' === meyvc_settings()->get( 'ai', $f, 'yes' ) ) {
					$ai_enabled = true;
					break;
				}
			}
		...
		$configured = class_exists( 'MEYVC_AI_Client' ) && MEYVC_AI_Client::is_configured();
		return array(
	...
	private static function check_webhook_configuration() {
		if ( ! class_exists( 'MEYVC_Webhook' ) || ! function_exists( 'meyvc_settings' ) ) {
			return array(
		...
		$endpoints = MEYVC_Webhook::get_endpoints();
		$active    = 0;
	...
	private static function check_klaviyo_integration() {
		$int_settings = function_exists( 'meyvc_settings' ) ? meyvc_settings()->get_group( 'integrations' ) : array();
		...
		$klaviyo_key  = ! empty( $int_settings['klaviyo_api_key_enc'] ) && class_exists( 'MEYVC_Security' )
			? MEYVC_Security::decrypt_secret( $int_settings['klaviyo_api_key_enc'] )
			: '';
	...
	private static function check_mailchimp_integration() {
		...
		$mc_key     = $mc_key_enc !== '' && class_exists( 'MEYVC_Security' )
			? MEYVC_Security::decrypt_secret( $mc_key_enc )
			: '';
		$ok         = $mc_key !== '' && $mc_list !== '';
		return array(
			'label'   => __( 'Mailchimp integration', 'meyvora-convert' ),
	...
	private static function check_asset_loading_mode() {
		...
		return array(
			...
			'status'  => 'ok',
			'message'  => $mode,
			'detail'  => __( 'Assets load only where CRO features are active unless overridden by meyvc_should_enqueue_assets.', 'meyvora-convert' ),
		);
	}
	...
	private static function check_cron() {
		...
		$meyvc_hooks = array( 'meyvc_process_background_queue', 'meyvc_cleanup_old_events', 'meyvc_aggregate_daily_stats' );
		$found = array();
		foreach ( $meyvc_hooks as $hook ) {
	...
	private static function check_cache_plugins() {
		...
		return array(
			...
			'detail'  => __( 'Ensure REST and campaign endpoints are not heavily cached; exclude MEYVC_Visitor_State cookie from cache key if applicable.', 'meyvora-convert' ),
		);
	}
	...
	public static function build_report( $checks = null ) {
		...
		$lines[] = 'Meyvora Convert: ' . ( defined( 'MEYVC_VERSION' ) ? MEYVC_VERSION : 'n/a' );
