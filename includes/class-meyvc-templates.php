...
/**
 * MEYVC_Templates class.
 *
 * Manages popup templates with previews and configurations.
 */
class MEYVC_Templates {
	...
	public static function get_all() {
		...
		return apply_filters( 'meyvc_templates', $templates );
	}
	...
	public static function get_available_for_builder() {
		$popup_dir = MEYVC_PLUGIN_DIR . 'templates/popups/';
		$all       = self::get_all();
		$available = array();
		...
		return apply_filters( 'meyvc_templates_available_for_builder', $available );
	}
	...
	public static function get_template_file( $template_key ) {
		...
		$plugin_file = MEYVC_PLUGIN_DIR . 'templates/popups/' . $template_key . '.php';
		if ( file_exists( $plugin_file ) ) {
			return $plugin_file;
		}

		// Fallback to centered
		return MEYVC_PLUGIN_DIR . 'templates/popups/centered.php';
	}
	...
	public static function get_brand_override_style_string( $override ) {
		...
		if ( ! empty( $override['primary_color'] ) && sanitize_hex_color( $override['primary_color'] ) ) {
			...
			$vars[] = '--meyvc-primary: ' . $hex;
			$vars[] = '--meyvc-primary-color: ' . $hex;
		}
		if ( ! empty( $override['secondary_color'] ) && sanitize_hex_color( $override['secondary_color'] ) ) {
			$vars[] = '--meyvc-secondary-color: ' . sanitize_hex_color( $override['secondary_color'] );
		}
		if ( isset( $override['button_radius'] ) && $override['button_radius'] !== '' ) {
			$px = (int) $override['button_radius'] . 'px';
			$vars[] = '--meyvc-radius: ' . $px;
			$vars[] = '--meyvc-button-radius: ' . $px;
		}
		if ( isset( $override['font_size_scale'] ) && $override['font_size_scale'] !== '' ) {
			...
			$vars[] = '--meyvc-font-size-scale: ' . $scale;
		}
		return implode( '; ', $vars );
	}
	...
	public static function render_campaign_preview_html( array $data ) {
		...
		$path = MEYVC_PLUGIN_DIR . 'templates/popups/' . $template . '.php';
		if ( ! file_exists( $path ) ) {
			$path = MEYVC_PLUGIN_DIR . 'templates/popups/centered.php';
		}
