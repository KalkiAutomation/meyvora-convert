 ...
 */
class MEYVC_Decision_Engine {

	/**
	 * Singleton instance.
	 *
	 * @var MEYVC_Decision_Engine|null
	 */
	 ...
	 *
	 * @param MEYVC_Context      $context       Request/visit context.
	 * @param MEYVC_Visitor_State $visitor_state Visitor state (cookie/session).
	 ...
	 * @return MEYVC_Decision
	 */
	public function decide( MEYVC_Context $context, MEYVC_Visitor_State $visitor_state, array $intent_signals = array(), $trigger_type = '', array $options = array() ) {
		$decision = new MEYVC_Decision( false, null, '', 'pending' );
		...
		// Step 1b: UX — do not show while user is typing or a form field is focused (signals from meyvc-signals.js).
		if ( class_exists( 'MEYVC_UX_Rules' ) && ! empty( $intent_signals ) ) {
			$ux = new MEYVC_UX_Rules();
		...
		$campaigns = apply_filters( 'meyvc_decision_active_campaigns', $campaigns, $context, $visitor_state );
		if ( empty( $campaigns ) ) {
		...
		$rule_engine = new MEYVC_Rule_Engine();
		$eligible = array();
		...
		if ( in_array( $trigger_type, $trigger_only_types, true ) ) {
			...
			$intent_scorer = new MEYVC_Intent_Scorer();
			foreach ( $eligible as $campaign ) {
		...
		if ( class_exists( 'MEYVC_AB_Test' ) && class_exists( 'MEYVC_Campaign_Model' ) ) {
			...
			if ( $active_test && ! empty( $active_test->id ) ) {
				...
				if ( $variation && ! empty( $variation->campaign_data ) ) {
					...
					if ( is_array( $campaign_row ) ) {
						$campaign_to_show = MEYVC_Campaign_Model::from_db_row( $campaign_row );
	...
	public function check_consent() {
		...
		$enabled = meyvc_settings()->get( 'general', 'plugin_enabled', true );
		if ( ! $enabled ) {
			return false;
		}
		return (bool) apply_filters( 'meyvc_consent_allows_popup', true );
	}
	 ...
	 * @param MEYVC_Visitor_State $visitor_state Visitor state.
	 ...
	 */
	public function check_visitor_suppression( MEYVC_Context $context, MEYVC_Visitor_State $visitor_state, array $options = array() ) {
		// Admin exclusion (optional; skip when debug mode is on so shop owners can test while logged in).
		if ( function_exists( 'meyvc_settings' ) && meyvc_settings()->get( 'general', 'exclude_admins', false ) ) {
			if ( $context->get( 'user.is_admin', false ) ) {
				$debug_on = meyvc_settings()->get( 'general', 'debug_mode', false );
				if ( ! $debug_on && apply_filters( 'meyvc_exclude_admins_from_campaigns', true, $context ) ) {
		...
		if ( empty( $options['ignore_session_popup_cap'] ) && function_exists( 'meyvc_settings' ) ) {
		...
		if ( $last_conversion !== null ) {
			$window = (int) apply_filters( 'meyvc_conversion_suppression_window', DAY_IN_SECONDS );
	...
	/**
	 * Get active campaigns from database as MEYVC_Campaign_Model instances.
	 *
	 * When {@see MEYVC_Query_Optimizer} is available and context is passed, uses cache + PHP pre-filter
	 * (schedule, page type, device) before building models.
	 *
	 * @param MEYVC_Context|null $context Optional. Request context for query optimizer.
	...
	public function get_active_campaigns( $context = null ) {
		...
		$rows = null;
		if ( class_exists( 'MEYVC_Query_Optimizer' ) && is_object( $context ) && method_exists( $context, 'get' ) ) {
			$filtered = MEYVC_Query_Optimizer::get_campaigns_for_decision( $context );
			if ( $filtered !== null ) {
				$rows = $filtered;
			}
		}
		if ( $rows === null ) {
			$rows = MEYVC_Campaign::get_all( array( 'status' => 'active' ) );
		}
		if ( ! is_array( $rows ) ) {
			return array();
		}
	...
	private function get_single_active_campaign( $id ) {
		$id = (int) $id;
		if ( $id <= 0 || ! class_exists( 'MEYVC_Campaign' ) || ! class_exists( 'MEYVC_Campaign_Model' ) ) {
			return array();
		}
	 ...
	 * @param MEYVC_Context   $context      Context (behavior.time_on_page, behavior.scroll_depth, etc.).
	 * @param MEYVC_Decision  $decision     Decision object for logging.
	 * @return array Filtered list of campaigns that match this trigger.
	 */
	private function filter_eligible_by_trigger( array $eligible, $trigger_type, MEYVC_Context $context, MEYVC_Decision $decision ) {
		$out = array();
	 ...
	 *
	 * @param MEYVC_Campaign_Model $campaign       Campaign model.
	 * @param MEYVC_Visitor_State   $visitor_state Visitor state.
	 * @return bool True if offer can be shown.
	 */
	public function check_offer_eligibility( $campaign, MEYVC_Visitor_State $visitor_state ) {
		...
		return (bool) apply_filters( 'meyvc_offer_eligibility', true, $campaign, $visitor_state );
	}

	/**
	 * Check frequency: max X per Y period, cooldown after conversion/click, then once_ever, once_per_session, etc.
	 *
	 * @param MEYVC_Campaign_Model|object $campaign       Campaign (with frequency_rules or display_rules).
	 * @param MEYVC_Visitor_State          $visitor_state Visitor state.
	 * @return bool True if frequency allows showing.
	 */
	public function check_frequency( $campaign, MEYVC_Visitor_State $visitor_state ) {
		$rules = array();
	 ...
	 *
	 * @return MEYVC_Campaign_Model|null
	 */
	public function get_fallback_campaign() {
		if ( ! function_exists( 'meyvc_settings' ) || ! class_exists( 'MEYVC_Campaign' ) || ! class_exists( 'MEYVC_Campaign_Model' ) ) {
			return null;
		}
		$fallback_id = (int) meyvc_settings()->get( 'general', 'fallback_campaign_id', 0 );
		if ( $fallback_id <= 0 ) {
			return null;
		}
		$row = MEYVC_Campaign::get( $fallback_id );
	...
	public function mark_shown( $campaign_id ) {
		...
		do_action( 'meyvc_campaign_shown_this_pageview', $campaign_id );
	}
	...
	private function targeting_rules_for_engine( $campaign ) {
		...
		if ( class_exists( 'MEYVC_Geo' ) && ! empty( $tr['geo_countries'] ) && is_array( $tr['geo_countries'] ) ) {
			$geo = MEYVC_Geo::normalize_countries( $tr['geo_countries'] );
			if ( ! empty( $geo ) ) {
		...
		// Cart status (builder saves cart_status; must map here — was only handled in legacy MEYVC_Targeting).
 ...
 *
 * Use: meyvc_decide()->decide( $context, $visitor_state, $intent_signals, $trigger_type, $options )
 *
 * @return MEYVC_Decision_Engine
 */
function meyvc_decide() {
	return MEYVC_Decision_Engine::get_instance();
}
