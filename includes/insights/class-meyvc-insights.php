...
class MEYVC_Insights {
	...
	public static function get_insights( $days = self::DAYS ) {
		...
		$cards = apply_filters( 'meyvc_insights_cards', $cards, $days );
		if ( ! is_array( $cards ) ) {
			$cards = array();
		}
	...
	private static function get_abandoned_recovery_snapshot( string $from, string $to ): array {
		$cached = MEYVC_Database::object_cache_get( 'insights_ab_recovery', array( $from, $to ) );
		if ( false !== $cached && is_array( $cached ) ) {
			return $cached;
		}
		global $wpdb;
		$t = $wpdb->prefix . 'meyvc_abandoned_carts';
		if ( ! MEYVC_Database::table_exists( $t ) ) {
			$empty = array( 'abandoned' => 0, 'recovered' => 0, 'rate' => 0.0 );
			MEYVC_Database::object_cache_set( 'insights_ab_recovery', array( $from, $to ), $empty );
			return $empty;
		}
		$cache_key_abandoned = 'meyvora_meyvc_' . md5( serialize( array( 'ab_recovery_abandoned_count', $t, $from, $to ) ) );
		$abandoned           = wp_cache_get( $cache_key_abandoned, 'meyvora_meyvc' );
		if ( false === $abandoned ) {
			...
			wp_cache_set( $cache_key_abandoned, $abandoned, 'meyvora_meyvc', 300 );
		} else {
			$abandoned = (int) $abandoned;
		}
		$cache_key_recovered = 'meyvora_meyvc_' . md5( serialize( array( 'ab_recovery_recovered_count', $t, $from, $to ) ) );
		$recovered           = wp_cache_get( $cache_key_recovered, 'meyvora_meyvc' );
		if ( false === $recovered ) {
			...
			wp_cache_set( $cache_key_recovered, $recovered, 'meyvora_meyvc', 300 );
		} else {
		...
		MEYVC_Database::object_cache_set( 'insights_ab_recovery', array( $from, $to ), $out );
		return $out;
	}
	...
	public static function get_funnel_data( int $days = self::DAYS ): array {
		...
		$date_to   = wp_date( 'Y-m-d' );
		$date_from = wp_date( 'Y-m-d', strtotime( '-' . $days . ' days' ) );
		$cached    = MEYVC_Database::object_cache_get( 'insights_funnel', array( $days, $date_from, $date_to ) );
		if ( false !== $cached && is_array( $cached ) ) {
			return $cached;
		}
		global $wpdb;
		$t = $wpdb->prefix . 'meyvc_events';
		if ( ! MEYVC_Database::table_exists( $t ) ) {
			$empty = array(
			...
			MEYVC_Database::object_cache_set( 'insights_funnel', array( $days, $date_from, $date_to ), $empty );
			return $empty;
		}
		$cache_key_sessions_imp = 'meyvora_meyvc_' . md5( serialize( array( 'funnel_sessions_impression', $t, $date_from, $date_to ) ) );
		$sessions_imp           = wp_cache_get( $cache_key_sessions_imp, 'meyvora_meyvc' );
		if ( false === $sessions_imp ) {
			...
			wp_cache_set( $cache_key_sessions_imp, $sessions_imp, 'meyvora_meyvc', 300 );
		} else {
			$sessions_imp = (int) $sessions_imp;
		}
		$cache_key_clicks = 'meyvora_meyvc_' . md5( serialize( array( 'funnel_sessions_clicks', $t, $date_from, $date_to ) ) );
		$clicks           = wp_cache_get( $cache_key_clicks, 'meyvora_meyvc' );
		if ( false === $clicks ) {
			...
			wp_cache_set( $cache_key_clicks, $clicks, 'meyvora_meyvc', 300 );
		} else {
			$clicks = (int) $clicks;
		}
		$cache_key_emails = 'meyvora_meyvc_' . md5( serialize( array( 'funnel_emails_captured', $t, $date_from, $date_to ) ) );
		$emails           = wp_cache_get( $cache_key_emails, 'meyvora_meyvc' );
		if ( false === $emails ) {
			...
			wp_cache_set( $cache_key_emails, $emails, 'meyvora_meyvc', 300 );
		} else {
			$emails = (int) $emails;
		}
		$cache_key_orders = 'meyvora_meyvc_' . md5( serialize( array( 'funnel_orders_count', $t, $date_from, $date_to ) ) );
		$orders           = wp_cache_get( $cache_key_orders, 'meyvora_meyvc' );
		if ( false === $orders ) {
			...
			wp_cache_set( $cache_key_orders, $orders, 'meyvora_meyvc', 300 );
		} else {
			$orders = (int) $orders;
		}
		...
		MEYVC_Database::object_cache_set( 'insights_funnel', array( $days, $date_from, $date_to ), $out );
		return $out;
	}
	...
	public static function get_hourly_heatmap( int $days = self::DAYS ): array {
		...
		$date_to   = wp_date( 'Y-m-d' );
		$date_from = wp_date( 'Y-m-d', strtotime( '-' . $days . ' days' ) );
		$cached    = MEYVC_Database::object_cache_get( 'insights_hourly_heatmap', array( $days, $date_from, $date_to ) );
		if ( false !== $cached && is_array( $cached ) ) {
			return $cached;
		}
		global $wpdb;
		$t = $wpdb->prefix . 'meyvc_events';
		if ( ! MEYVC_Database::table_exists( $t ) ) {
			MEYVC_Database::object_cache_set( 'insights_hourly_heatmap', array( $days, $date_from, $date_to ), $matrix );
			return $matrix;
		}
		$cache_key_heatmap = 'meyvora_meyvc_' . md5( serialize( array( 'hourly_heatmap_conversions_by_dow_hr', $t, $date_from, $date_to ) ) );
		$rows              = wp_cache_get( $cache_key_heatmap, 'meyvora_meyvc' );
		if ( false === $rows ) {
			...
			wp_cache_set( $cache_key_heatmap, $rows, 'meyvora_meyvc', 300 );
		} else {
	...
	public static function get_offer_performance( int $days = self::DAYS ): array {
		...
		$date_to   = wp_date( 'Y-m-d' );
		$date_from = wp_date( 'Y-m-d', strtotime( '-' . $days . ' days' ) );
		$cached    = MEYVC_Database::object_cache_get( 'insights_offer_perf', array( $days, $date_from, $date_to ) );
		if ( false !== $cached && is_array( $cached ) ) {
			return $cached;
		}
		$logs      = $wpdb->prefix . 'meyvc_offer_logs';
		$offers    = $wpdb->prefix . 'meyvc_offers';
		if ( ! MEYVC_Database::table_exists( $logs ) || ! MEYVC_Database::table_exists( $offers ) ) {
			MEYVC_Database::object_cache_set( 'insights_offer_perf', array( $days, $date_from, $date_to ), array() );
			return array();
		}
		$cache_key_offer_logs_col = 'meyvora_meyvc_' . md5( serialize( array( 'offer_logs_action_column', $logs ) ) );
		$has_action_raw           = wp_cache_get( $cache_key_offer_logs_col, 'meyvora_meyvc' );
		if ( false === $has_action_raw ) {
			...
			wp_cache_set( $cache_key_offer_logs_col, $has_action_raw ? 1 : 0, 'meyvora_meyvc', 300 );
		}
		$has_action = (bool) (int) $has_action_raw;
		if ( $has_action ) {
			$cache_key_offer_perf_wa = 'meyvora_meyvc_' . md5( serialize( array( 'offer_perf_with_action', $logs, $date_from, $date_to ) ) );
			$rows                    = wp_cache_get( $cache_key_offer_perf_wa, 'meyvora_meyvc' );
			if ( false === $rows ) {
				$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above with wp_cache_get/set.
					$wpdb->prepare(
				...
				wp_cache_set( $cache_key_offer_perf_wa, $rows, 'meyvora_meyvc', 300 );
			} else {
				$rows = is_array( $rows ) ? $rows : array();
			}
		} else {
			$cache_key_offer_perf_na = 'meyvora_meyvc_' . md5( serialize( array( 'offer_perf_no_action', $logs, $date_from, $date_to ) ) );
			$rows                    = wp_cache_get( $cache_key_offer_perf_na, 'meyvora_meyvc' );
			if ( false === $rows ) {
				$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above with wp_cache_get/set.
					$wpdb->prepare(
						'SELECT l.offer_id, COUNT(*) AS applies,
				...
				wp_cache_set( $cache_key_offer_perf_na, $rows, 'meyvora_meyvc', 300 );
			} else {
				$rows = is_array( $rows ) ? $rows : array();
			}
		}
		...
		foreach ( is_array( $rows ) ? $rows : array() as $row ) {
			...
			$cache_key_offer_name = 'meyvora_meyvc_' . md5( serialize( array( 'offer_name_by_id', $offers, $oid ) ) );
			$name                 = wp_cache_get( $cache_key_offer_name, 'meyvora_meyvc' );
			if ( false === $name ) {
				...
				wp_cache_set( $cache_key_offer_name, $name, 'meyvora_meyvc', 300 );
			} else {
				$name = (string) $name;
			}
		...
		MEYVC_Database::object_cache_set( 'insights_offer_perf', array( $days, $date_from, $date_to ), $list );
		return $list;
	}
	...
	private static function sum_revenue_for_offer_orders( string $logs, int $offer_id, string $date_from, string $date_to ): float {
		$cached = MEYVC_Database::object_cache_get( 'insights_offer_rev_sum', array( $logs, $offer_id, $date_from, $date_to ) );
		if ( false !== $cached && is_numeric( $cached ) ) {
			return (float) $cached;
		}
		global $wpdb;
		$cache_key_rev_ids = 'meyvora_meyvc_' . md5( serialize( array( 'offer_rev_order_ids', $logs, $offer_id, $date_from, $date_to ) ) );
		$ids               = wp_cache_get( $cache_key_rev_ids, 'meyvora_meyvc' );
		if ( false === $ids ) {
			...
			wp_cache_set( $cache_key_rev_ids, $ids, 'meyvora_meyvc', 300 );
		} else {
		...
		MEYVC_Database::object_cache_set( 'insights_offer_rev_sum', array( $logs, $offer_id, $date_from, $date_to ), $sum );
		return $sum;
	}
	...
	public static function get_abandoned_cart_stats( int $days = self::DAYS ): array {
		...
		$date_to   = wp_date( 'Y-m-d' );
		$date_from = wp_date( 'Y-m-d', strtotime( '-' . $days . ' days' ) );
		$cached    = MEYVC_Database::object_cache_get( 'insights_ab_cart_stats', array( $days, $date_from, $date_to ) );
		if ( false !== $cached && is_array( $cached ) ) {
			return $cached;
		}
		$t         = $wpdb->prefix . 'meyvc_abandoned_carts';
		...
		if ( ! MEYVC_Database::table_exists( $t ) ) {
			MEYVC_Database::object_cache_set( 'insights_ab_cart_stats', array( $days, $date_from, $date_to ), $empty );
			return $empty;
		}

		$cache_key_total_ab = 'meyvora_meyvc_' . md5( serialize( array( 'ab_cart_total_abandoned', $t, $date_from, $date_to ) ) );
		$total_abandoned    = wp_cache_get( $cache_key_total_ab, 'meyvora_meyvc' );
		if ( false === $total_abandoned ) {
			...
			wp_cache_set( $cache_key_total_ab, $total_abandoned, 'meyvora_meyvc', 300 );
		} else {
			$total_abandoned = (int) $total_abandoned;
		}
		$cache_key_total_rec = 'meyvora_meyvc_' . md5( serialize( array( 'ab_cart_total_recovered', $t, $date_from, $date_to ) ) );
		$total_recovered     = wp_cache_get( $cache_key_total_rec, 'meyvora_meyvc' );
		if ( false === $total_recovered ) {
			...
			wp_cache_set( $cache_key_total_rec, $total_recovered, 'meyvora_meyvc', 300 );
		} else {
			$total_recovered = (int) $total_recovered;
		}
		$rate            = $total_abandoned > 0 ? round( ( $total_recovered / $total_abandoned ) * 100, 2 ) : 0.0;

		$cache_key_avg_cart = 'meyvora_meyvc_' . md5( serialize( array( 'ab_cart_avg_cart_total', $t, $date_from, $date_to ) ) );
		$avg                = wp_cache_get( $cache_key_avg_cart, 'meyvora_meyvc' );
		if ( false === $avg ) {
			...
			wp_cache_set( $cache_key_avg_cart, $avg, 'meyvora_meyvc', 300 );
		} else {
			$avg = is_numeric( $avg ) ? (float) $avg : 0.0;
		}
		$avg_val = is_numeric( $avg ) ? round( (float) $avg, 2 ) : 0.0;

		$cache_key_band_0_50 = 'meyvora_meyvc_' . md5( serialize( array( 'ab_cart_band_0_50', $t, $date_from, $date_to ) ) );
		$band_0_50           = wp_cache_get( $cache_key_band_0_50, 'meyvora_meyvc' );
		if ( false === $band_0_50 ) {
			...
			wp_cache_set( $cache_key_band_0_50, $band_0_50, 'meyvora_meyvc', 300 );
		} else {
			$band_0_50 = (int) $band_0_50;
		}
		$cache_key_band_50_100 = 'meyvora_meyvc_' . md5( serialize( array( 'ab_cart_band_50_100', $t, $date_from, $date_to ) ) );
		$band_50_100           = wp_cache_get( $cache_key_band_50_100, 'meyvora_meyvc' );
		if ( false === $band_50_100 ) {
			...
			wp_cache_set( $cache_key_band_50_100, $band_50_100, 'meyvora_meyvc', 300 );
		} else {
			$band_50_100 = (int) $band_50_100;
		}
		$cache_key_band_100_200 = 'meyvora_meyvc_' . md5( serialize( array( 'ab_cart_band_100_200', $t, $date_from, $date_to ) ) );
		$band_100_200           = wp_cache_get( $cache_key_band_100_200, 'meyvora_meyvc' );
		if ( false === $band_100_200 ) {
			...
			wp_cache_set( $cache_key_band_100_200, $band_100_200, 'meyvora_meyvc', 300 );
		} else {
			$band_100_200 = (int) $band_100_200;
		}
		$cache_key_band_200_plus = 'meyvora_meyvc_' . md5( serialize( array( 'ab_cart_band_200_plus', $t, $date_from, $date_to ) ) );
		$band_200_plus           = wp_cache_get( $cache_key_band_200_plus, 'meyvora_meyvc' );
		if ( false === $band_200_plus ) {
			...
			wp_cache_set( $cache_key_band_200_plus, $band_200_plus, 'meyvora_meyvc', 300 );
		} else {
		...
		MEYVC_Database::object_cache_set( 'insights_ab_cart_stats', array( $days, $date_from, $date_to ), $out );
		return $out;
	}
	...
	private static function recovery_rate_for_email_step( string $table, int $step, string $date_from, string $date_to ): ?float {
		...
		$cached = MEYVC_Database::object_cache_get( 'insights_recovery_email_step', $params );
		...
		if ( ! isset( $cols[ $step ] ) ) {
			MEYVC_Database::object_cache_set( 'insights_recovery_email_step', $params, array( '_nil' => true ) );
			return null;
		}
		$col = $cols[ $step ];
		$cache_key_recovery_sent = 'meyvora_meyvc_' . md5( serialize( array( 'recovery_email_sent_count', $table, $col, $date_from, $date_to, $step ) ) );
		$sent                    = wp_cache_get( $cache_key_recovery_sent, 'meyvora_meyvc' );
		if ( false === $sent ) {
			...
			wp_cache_set( $cache_key_recovery_sent, $sent, 'meyvora_meyvc', 300 );
		} else {
			$sent = (int) $sent;
		}
		if ( $sent < 1 ) {
		...
		$cache_key_recovery_rec = 'meyvora_meyvc_' . md5( serialize( array( 'recovery_email_recovered_count', $table, $col, $date_from, $date_to, $step ) ) );
		$rec                    = wp_cache_get( $cache_key_recovery_rec, 'meyvora_meyvc' );
		if ( false === $rec ) {
			...
			wp_cache_set( $cache_key_recovery_rec, $rec, 'meyvora_meyvc', 300 );
		} else {
			$rec = (int) $rec;
		}
		$rate = round( ( $rec / $sent ) * 100, 2 );
		MEYVC_Database::object_cache_set( 'insights_recovery_email_step', $params, array( '_nil' => false, 'f' => $rate ) );
		return $rate;
	}
	...
	public static function bust_insights_cache(): void {
		$days_list = apply_filters(
			'meyvc_insights_cached_day_keys',
		...
		foreach ( $days_list as $d ) {
			$d = absint( $d );
			if ( $d > 0 ) {
				delete_transient( 'meyvc_insights_' . $d . 'd' );
			}
		}
	}
	...
	public static function invalidate_attribution_cache(): void {
		update_option( 'meyvc_attribution_cache_epoch', (int) get_option( 'meyvc_attribution_cache_epoch', 0 ) + 1 );
	...
	private static function get_attribution_cache_key( $cache_key_suffix ) {
		$epoch = (int) get_option( 'meyvc_attribution_cache_epoch', 0 );
		return 'meyvc_attribution_' . $cache_key_suffix . '_v' . $epoch;
	}
	...
	public static function get_attribution() {
		$window_days = (int) apply_filters( 'meyvc_tracking_attribution_window_days', 7 );
		...
		try {
			...
			$ttl    = (int) apply_filters( 'meyvc_insights_cache_ttl', 600 );
			$ttl    = $ttl >= 60 ? $ttl : 600;
	...
	private static function get_attribution_uncached( $window_days ) {
		$epoch = (int) get_option( 'meyvc_attribution_cache_epoch', 0 );
		$ck    = MEYVC_Database::object_cache_get( 'insights_attr_uncached', array( (int) $window_days, $epoch ) );
		...
		if ( ! MEYVC_Database::table_exists( $events_table ) ) {
			$empty = self::attribution_empty_response( $window_days, true );
			MEYVC_Database::object_cache_set( 'insights_attr_uncached', array( (int) $window_days, $epoch ), $empty );
			return $empty;
		}

		$cache_key_attr_conv = 'meyvora_meyvc_' . md5( serialize( array( 'attr_total_conversions', $events_table, $date_from, $date_to ) ) );
		$total_conversions     = wp_cache_get( $cache_key_attr_conv, 'meyvora_meyvc' );
		if ( false === $total_conversions ) {
			...
			wp_cache_set( $cache_key_attr_conv, $total_conversions, 'meyvora_meyvc', 300 );
		} else {
			$total_conversions = (int) $total_conversions;
		}
		$cache_key_attr_imp = 'meyvora_meyvc_' . md5( serialize( array( 'attr_total_impressions', $events_table, $date_from, $date_to ) ) );
		$total_impressions  = wp_cache_get( $cache_key_attr_imp, 'meyvora_meyvc' );
		if ( false === $total_impressions ) {
			...
			wp_cache_set( $cache_key_attr_imp, $total_impressions, 'meyvora_meyvc', 300 );
		} else {
		...
		$top_offers = array();
		if ( MEYVC_Database::table_exists( $logs ) && MEYVC_Database::table_exists( $offers ) ) {
			$cache_key_attr_offer_col = 'meyvora_meyvc_' . md5( serialize( array( 'offer_logs_action_column_attr', $logs ) ) );
			$has_action_raw_attr      = wp_cache_get( $cache_key_attr_offer_col, 'meyvora_meyvc' );
			if ( false === $has_action_raw_attr ) {
				...
				wp_cache_set( $cache_key_attr_offer_col, $has_action_raw_attr ? 1 : 0, 'meyvora_meyvc', 300 );
			}
		...
		$ab_var_table   = $wpdb->prefix . 'meyvc_ab_variations';
		if ( MEYVC_Database::table_exists( $ab_tests_table ) && MEYVC_Database::table_exists( $ab_var_table ) ) {
			$cache_key_top_ab = 'meyvora_meyvc_' . md5( serialize( array( 'attr_top_ab_tests', $ab_tests_table, $ab_var_table ) ) );
			$rows             = wp_cache_get( $cache_key_top_ab, 'meyvora_meyvc' );
			if ( false === $rows ) {
				$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above with wp_cache_get/set.
					$wpdb->prepare(
				...
				wp_cache_set( $cache_key_top_ab, $rows, 'meyvora_meyvc', 300 );
			} else {
				$rows = is_array( $rows ) ? $rows : array();
			}
			if ( $wpdb->last_error ) {
			...
			foreach ( is_array( $rows ) ? $rows : array() as $row ) {
				...
				if ( $conv > 0 ) {
					$top_ab_tests[] = array(
						'name'        => $row['name'] ?? __( 'Unknown', 'meyvora-convert' ),
						'conversions' => $conv,
						'url'         => admin_url( 'admin.php?page=meyvc-ab-test-view&id=' . (int) $row['id'] ),
					);
				}
			}
		}
		...
		MEYVC_Database::object_cache_set( 'insights_attr_uncached', array( (int) $window_days, $epoch ), $attr_out );
		return $attr_out;
	}
	...
	private static function get_top_offers_attribution( $events_table, $logs, $offers, $date_from, $date_to, $require_applied_action = false ) {
		...
		$cached  = MEYVC_Database::object_cache_get( 'insights_top_offers_attr', $top_key );
		if ( false !== $cached && is_array( $cached ) ) {
			return $cached;
		}
		global $wpdb;
		...
		$cache_key_ctype = 'meyvora_meyvc_' . md5( serialize( array( 'events_source_type_column_type', DB_NAME, $events_table, 'source_type' ) ) );
		$ctype_raw         = wp_cache_get( $cache_key_ctype, 'meyvora_meyvc' );
		if ( false === $ctype_raw ) {
			...
			wp_cache_set( $cache_key_ctype, $ctype, 'meyvora_meyvc', 300 );
		} else {
		...
		if ( $has_offer_type ) {
			$cache_key_top_ev = 'meyvora_meyvc_' . md5( serialize( array( 'top_offers_events_by_offer', $events_table, $date_from, $date_to ) ) );
			$rows             = wp_cache_get( $cache_key_top_ev, 'meyvora_meyvc' );
			if ( false === $rows ) {
				$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above with wp_cache_get/set.
					$wpdb->prepare(
				...
				wp_cache_set( $cache_key_top_ev, $rows, 'meyvora_meyvc', 300 );
			} else {
				$rows = is_array( $rows ) ? $rows : array();
			}
		} else {
		...
		foreach ( is_array( $rows ) ? $rows : array() as $r ) {
			...
			$cache_key_nm_attr = 'meyvora_meyvc_' . md5( serialize( array( 'offer_name_row_attr', $offers, $offer_id ) ) );
			$name_cache        = wp_cache_get( $cache_key_nm_attr, 'meyvora_meyvc' );
			if ( false === $name_cache ) {
				...
				$stored_nm = null === $name_row ? array( '__meyvc_null_row__' => true ) : $name_row;
				wp_cache_set( $cache_key_nm_attr, $stored_nm, 'meyvora_meyvc', 300 );
			} elseif ( is_array( $name_cache ) && isset( $name_cache['__meyvc_null_row__'] ) ) {
				$name_row = null;
			} else {
			...
			if ( $require_applied_action ) {
				$cache_key_ap_applied = 'meyvora_meyvc_' . md5( serialize( array( 'offer_applies_applied', $logs, $offer_id, $date_from, $date_to ) ) );
				$applies              = wp_cache_get( $cache_key_ap_applied, 'meyvora_meyvc' );
				if ( false === $applies ) {
					...
					wp_cache_set( $cache_key_ap_applied, $applies, 'meyvora_meyvc', 300 );
				} else {
					$applies = (int) $applies;
				}
			} else {
				$cache_key_ap_any = 'meyvora_meyvc_' . md5( serialize( array( 'offer_applies_any', $logs, $offer_id, $date_from, $date_to ) ) );
				$applies            = wp_cache_get( $cache_key_ap_any, 'meyvora_meyvc' );
				if ( false === $applies ) {
					$applies = (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above with wp_cache_get/set.
						$wpdb->prepare(
					...
					wp_cache_set( $cache_key_ap_any, $applies, 'meyvora_meyvc', 300 );
				} else {
					$applies = (int) $applies;
				}
			}
		...
		if ( empty( $list ) ) {
			if ( $require_applied_action ) {
				$cache_key_fb_applied = 'meyvora_meyvc_' . md5( serialize( array( 'top_offers_fallback_applied', $logs, $offers, $date_from, $date_to ) ) );
				$rows                 = wp_cache_get( $cache_key_fb_applied, 'meyvora_meyvc' );
				if ( false === $rows ) {
					$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above with wp_cache_get/set.
						$wpdb->prepare(
					...
					wp_cache_set( $cache_key_fb_applied, $rows, 'meyvora_meyvc', 300 );
				} else {
					$rows = is_array( $rows ) ? $rows : array();
				}
			} else {
				$cache_key_fb_any = 'meyvora_meyvc_' . md5( serialize( array( 'top_offers_fallback_any', $logs, $offers, $date_from, $date_to ) ) );
				$rows             = wp_cache_get( $cache_key_fb_any, 'meyvora_meyvc' );
				if ( false === $rows ) {
					$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above with wp_cache_get/set.
						$wpdb->prepare(
							'SELECT o.id AS offer_id, o.name, COUNT(l.id) AS applies FROM %i l
					...
					wp_cache_set( $cache_key_fb_any, $rows, 'meyvora_meyvc', 300 );
				} else {
					$rows = is_array( $rows ) ? $rows : array();
				}
			}
		...
		MEYVC_Database::object_cache_set( 'insights_top_offers_attr', $top_key, $list );
		return $list;
	}
	...
	private static function insight_top_campaign( int $days ): ?array {
		...
		return self::make_card(
			...
			admin_url( 'admin.php?page=meyvc-campaign-edit&campaign_id=' . (int) $top['id'] ),
		...
		foreach ( is_array( $rows ) ? $rows : array() as $row ) {
			...
			if ( $imp >= 50 && $conv < 2 && $rate < 3 ) {
				return self::make_card(
					...
					admin_url( 'admin.php?page=meyvc-campaign-edit&campaign_id=' . (int) ( $row['id'] ?? 0 ) ),
	...
	private static function insight_best_time( int $days ): ?array {
		...
		return self::make_card(
			...
			admin_url( 'admin.php?page=meyvc-campaigns' ),
		...
		foreach ( self::get_offer_performance( $days ) as $o ) {
			if ( (int) $o['applies'] >= 10 && (int) $o['orders_with_coupon'] < 1 ) {
				return self::make_card(
					...
					admin_url( 'admin.php?page=meyvc-offers' ),
	...
	private static function insight_high_value_abandoned( int $days ): ?array {
		...
		return self::make_card(
			...
			admin_url( 'admin.php?page=meyvc-abandoned-cart' ),
	...
	private static function insight_ab_test_ready( int $days ): ?array {
		...
		$ab_table        = $wpdb->prefix . 'meyvc_ab_tests';
		if ( ! MEYVC_Database::table_exists( $campaigns_table ) || ! MEYVC_Database::table_exists( $ab_table ) ) {
			return null;
		}
		$cache_key_ab_ready = 'meyvora_meyvc_' . md5( serialize( array( 'ab_test_ready_campaign', $campaigns_table, $ab_table ) ) );
		$row_cache           = wp_cache_get( $cache_key_ab_ready, 'meyvora_meyvc' );
		if ( false === $row_cache ) {
			...
			$stored_ab = null === $row ? array( '__meyvc_null_row__' => true ) : $row;
			wp_cache_set( $cache_key_ab_ready, $stored_ab, 'meyvora_meyvc', 300 );
		} elseif ( is_array( $row_cache ) && isset( $row_cache['__meyvc_null_row__'] ) ) {
			$row = null;
		} else {
		...
		return self::make_card(
			...
			admin_url( 'admin.php?page=meyvc-ab-test-new&campaign_id=' . (int) $row['id'] ),
	...
	private static function insight_booster_no_lift( int $days ): ?array {
		...
		$date_from = wp_date( 'Y-m-d', strtotime( '-' . self::normalize_days( $days ) . ' days' ) );
		global $wpdb;
		$ev = $wpdb->prefix . 'meyvc_events';
		if ( ! MEYVC_Database::table_exists( $ev ) ) {
			return null;
		}
		$cache_key_booster_imp = 'meyvora_meyvc_' . md5( serialize( array( 'booster_shipping_impressions', $ev, $date_from, $date_to ) ) );
		$imp                   = wp_cache_get( $cache_key_booster_imp, 'meyvora_meyvc' );
		if ( false === $imp ) {
			...
			wp_cache_set( $cache_key_booster_imp, $imp, 'meyvora_meyvc', 300 );
		} else {
			$imp = (int) $imp;
		}
		$cache_key_booster_int = 'meyvora_meyvc_' . md5( serialize( array( 'booster_shipping_interactions', $ev, $date_from, $date_to ) ) );
		$int                   = wp_cache_get( $cache_key_booster_int, 'meyvora_meyvc' );
		if ( false === $int ) {
			...
			wp_cache_set( $cache_key_booster_int, $int, 'meyvora_meyvc', 300 );
		} else {
			$int = (int) $int;
		}
		if ( $imp < 30 ) {
		...
		return self::make_card(
			...
			admin_url( 'admin.php?page=meyvc-boosters' ),
	...
	private static function insight_checkout_drop( int $days ): ?array {
		...
		$cache_key_chk_imp = 'meyvora_meyvc_' . md5( serialize( array( 'checkout_impressions', $ev, $date_from, $date_to ) ) );
		$imp               = wp_cache_get( $cache_key_chk_imp, 'meyvora_meyvc' );
		if ( false === $imp ) {
			$imp = (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above with wp_cache_get/set.
				$wpdb->prepare(
			...
			wp_cache_set( $cache_key_chk_imp, $imp, 'meyvora_meyvc', 300 );
		} else {
			$imp = (int) $imp;
		}
		$cache_key_chk_conv = 'meyvora_meyvc_' . md5( serialize( array( 'checkout_conversions', $ev, $date_from, $date_to ) ) );
		$conv               = wp_cache_get( $cache_key_chk_conv, 'meyvora_meyvc' );
		if ( false === $conv ) {
			...
			wp_cache_set( $cache_key_chk_conv, $conv, 'meyvora_meyvc', 300 );
		} else {
			$conv = (int) $conv;
		}
		if ( $imp < 50 ) {
		...
		return self::make_card(
			...
			admin_url( 'admin.php?page=meyvc-checkout' ),
	...
	private static function insight_email_growth( int $days ): ?array {
		...
		return self::make_card(
			...
			admin_url( 'admin.php?page=meyvc-analytics' ),
	...
	private static function insight_device_gap( int $days ): ?array {
		...
		$cache_key_device_gap = 'meyvora_meyvc_' . md5( serialize( array( 'device_gap_by_device', $ev, $date_from, $date_to ) ) );
		$rows                 = wp_cache_get( $cache_key_device_gap, 'meyvora_meyvc' );
		if ( false === $rows ) {
			$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above with wp_cache_get/set.
				$wpdb->prepare(
					"SELECT device_type,
			...
			wp_cache_set( $cache_key_device_gap, $rows, 'meyvora_meyvc', 300 );
		} else {
			$rows = is_array( $rows ) ? $rows : array();
		}
		$m_rate = $d_rate = null;
