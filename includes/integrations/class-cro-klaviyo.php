...
class CRO_Klaviyo {
	...
	public function __construct() {
		$settings = cro_settings()->get_group( 'integrations' );
		if ( empty( $settings['klaviyo_enabled'] ) ) {
			return;
		}
		add_action( 'cro_email_captured', array( $this, 'on_email' ), 30, 3 );
		add_action( 'cro_abandoned_cart_created', array( $this, 'on_abandoned' ), 30, 1 );
	}
	...
	private function subscribe( string $email, string $source ): void {
		...
		if ( $list_id === '' || $key_enc === '' || ! class_exists( 'CRO_Security' ) ) {
			return;
		}

		$api_key = CRO_Security::decrypt_secret( $key_enc );
		if ( $api_key === '' ) {
			return;
		}
		...
		if ( is_wp_error( $profile_response ) ) {
			if ( class_exists( 'CRO_Error_Handler' ) ) {
				CRO_Error_Handler::log( 'KLAVIYO', 'Profile upsert failed: ' . $profile_response->get_error_message(), array( 'email' => $email ) );
			}
			return;
		}
		...
		if ( 409 === $profile_code ) {
			...
			if ( class_exists( 'CRO_Error_Handler' ) ) {
				CRO_Error_Handler::log( 'KLAVIYO', 'Unexpected profile status: ' . $profile_code, array( 'email' => $email ) );
			}
			return;
		}
		...
		if ( is_wp_error( $sub_response ) && class_exists( 'CRO_Error_Handler' ) ) {
			CRO_Error_Handler::log( 'KLAVIYO', 'List subscribe failed: ' . $sub_response->get_error_message(), array( 'email' => $email, 'profile_id' => $profile_id ) );
		}
	}
}
