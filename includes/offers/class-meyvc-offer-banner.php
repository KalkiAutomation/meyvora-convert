...
/**
 * MEYVC_Offer_Banner class.
 */
class MEYVC_Offer_Banner {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_meyvc_apply_offer_coupon', array( $this, 'ajax_apply_offer_coupon' ) );
		add_action( 'wp_ajax_nopriv_meyvc_apply_offer_coupon', array( $this, 'ajax_apply_offer_coupon' ) );
	}
	...
	private function get_settings() {
		...
		return meyvc_settings()->get_offer_banner_settings();
	}
	...
	private function render_banner_markup( $context ) {
		...
		$result = MEYVC_Offer_Engine::get_best_offer_with_coupon( null );
		...
		$nonce = wp_create_nonce( 'meyvc_apply_offer_coupon' );
		$url   = admin_url( 'admin-ajax.php' );
		$render_context = array( 'context' => $context, 'offer' => $offer, 'coupon_code' => $code );
		do_action( 'meyvc_frontend_before_render', 'offer_banner', $render_context );
		?>
		<div class="meyvc-offer-banner" data-context="<?php echo esc_attr( $context ); ?>" data-nonce="<?php echo esc_attr( $nonce ); ?>" data-ajax-url="<?php echo esc_attr( $url ); ?>"<?php echo $can_apply && $code ? ' data-coupon-code="' . esc_attr( $code ) . '"' : ''; ?>>
			<p class="meyvc-offer-banner__text meyvc-offer-banner__headline"><?php echo esc_html( $headline ); ?></p>
			<?php if ( $description !== '' ) : ?>
				<p class="meyvc-offer-banner__description"><?php echo esc_html( $description ); ?></p>
			<?php endif; ?>
			<?php if ( $can_apply ) : ?>
			<p class="meyvc-offer-banner__actions">
				<button type="button" class="button meyvc-offer-apply-coupon"><?php esc_html_e( 'Apply coupon', 'meyvora-convert' ); ?></button>
			</p>
			<?php else : ?>
			<p class="meyvc-offer-banner__applied"><?php esc_html_e( 'Discount applied.', 'meyvora-convert' ); ?></p>
			<?php endif; ?>
			<p class="meyvc-offer-banner__msg meyvc-offer-banner__msg--success" style="display:none;"></p>
			<p class="meyvc-offer-banner__msg meyvc-offer-banner__msg--error" style="display:none;"></p>
		</div>
		<?php
		do_action( 'meyvc_frontend_after_render', 'offer_banner', $render_context );
	}
	...
	public function maybe_render_cart_banner() {
		...
		if ( $max_24h > 0 && class_exists( 'MEYVC_Visitor_State' ) ) {
			MEYVC_Visitor_State::get_instance()->record_banner_show( 'offer' );
		}
	}
	...
	public function enqueue_assets() {
		...
		wp_enqueue_script(
			'meyvc-offer-banner',
			MEYVC_PLUGIN_URL . 'public/js/meyvc-offer-banner' . meyvc_asset_min_suffix() . '.js',
			array( 'jquery' ),
			MEYVC_VERSION,
			true
		);
		// Styles: meyvc-classic-cart-checkout.css is enqueued by MEYVC_Classic_Cart_Checkout when offer/cart/checkout feature is on.
	}
	...
	public function ajax_apply_offer_coupon() {
		check_ajax_referer( 'meyvc_apply_offer_coupon', 'nonce' );
		...
		if ( $code !== '' ) {
			...
			$code = class_exists( 'MEYVC_Offer_Engine' ) ? MEYVC_Offer_Engine::get_or_create_coupon_for_best_offer() : null;
			if ( empty( $code ) ) {
