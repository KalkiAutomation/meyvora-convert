...
/**
 * MEYVC_Offer_Engine class.
 */
class MEYVC_Offer_Engine {
	 ...
	 *
	 * @return array Context array. Filtered by meyvc_offer_context.
	 */
	public static function build_context() {
		...
		// Visitor ID (MEYVC_Visitor_State).
		...
		return apply_filters( 'meyvc_offer_context', $context );
	}
	...
	/**
	 * Get registered rule evaluators (from MEYVC_Offer_Rules + filter).
	 * Filter: meyvc_offer_rules — modify or extend rule_key => callable( $value, $context ).
	 *
	 * @return array<string, callable>
	 */
	public static function get_rule_evaluators() {
		$evaluators = class_exists( 'MEYVC_Offer_Rules' ) ? MEYVC_Offer_Rules::get_evaluators() : array();
		return apply_filters( 'meyvc_offer_rules', $evaluators );
	}
	...
	public static function evaluate_condition( $key, $value, array $context ) {
		...
		return class_exists( 'MEYVC_Offer_Rules' )
			? MEYVC_Offer_Rules::evaluate( $key, $value, $context, $evaluators )
			: true;
	}
	...
	public static function evaluate_conditions( array $conditions, array $context ) {
		...
		$evaluators = self::get_rule_evaluators();
		return class_exists( 'MEYVC_Offer_Rules' )
			? MEYVC_Offer_Rules::evaluate_all( $conditions, $context, $evaluators )
			: true;
	}
	 ...
	 * Shared evaluation path for admin Test panel and REST GET /meyvc/v1/offer.
	 *
	...
	public static function get_offer_stable_key( $offer ) {
		...
		if ( isset( $offer->meyvc_stable_key ) && is_string( $offer->meyvc_stable_key ) && $offer->meyvc_stable_key !== '' ) {
			return $offer->meyvc_stable_key;
		}
		if ( isset( $offer->id ) ) {
	...
	public static function get_offer_conflict_partner_keys( $offer ) {
		...
		if ( isset( $offer->meyvc_conflict_partner_keys ) && is_array( $offer->meyvc_conflict_partner_keys ) ) {
			return $offer->meyvc_conflict_partner_keys;
		}
		if ( class_exists( 'MEYVC_Offer_Model' ) ) {
			$ids = MEYVC_Offer_Model::normalize_conflict_ids_on_row( $offer );
			$keys = array();
			foreach ( $ids as $id ) {
				$keys[] = 'db:' . (int) $id;
			}
	...
	/**
	 * Get active offers: from option meyvc_dynamic_offers (if set) or from DB (MEYVC_Offer_Model).
	 * Option-based offers use numeric ids 9000, 9001, ... for coupon generation.
	 *
	 * Single source of truth: when the admin Offers drawer saves data, it updates `meyvc_dynamic_offers`.
	...
	public static function get_active_offers() {
		$option_offers = get_option( 'meyvc_dynamic_offers', array() );
		...
		if ( class_exists( 'MEYVC_Offer_Model' ) ) {
			return MEYVC_Offer_Model::get_active();
		}
		return array();
	}

	/**
	 * Convert meyvc_dynamic_offers option array to offer objects for the engine.
	 *
	...
	public static function option_offers_to_objects( array $option_offers ) {
		...
		foreach ( $option_offers as $i => $cfg ) {
			...
			$obj->meyvc_stable_key    = $opt_id !== '' ? 'opt:' . $opt_id : 'opt_slot:' . $i;
			...
			$obj->meyvc_conflict_partner_keys = array_values( array_unique( $partner_keys ) );
			$out[] = $obj;
		}
	 ...
	 *
	 * @param array $cfg Option config (same keys as meyvc_dynamic_offers item).
	...
	public static function option_single_to_object( array $cfg, $id ) {
		...
		$opt_id                = isset( $cfg['id'] ) ? sanitize_text_field( (string) $cfg['id'] ) : '';
		$obj->meyvc_stable_key   = $opt_id !== '' ? 'opt:' . $opt_id : 'opt_slot:' . max( 0, $slot_index );
		$conf_ids              = isset( $cfg['conflict_offer_ids'] ) && is_array( $cfg['conflict_offer_ids'] ) ? $cfg['conflict_offer_ids'] : array();
		$partner_keys          = array();
		foreach ( $conf_ids as $cid ) {
			$cid = is_scalar( $cid ) ? sanitize_text_field( (string) $cid ) : '';
	...
	public static function get_best_offer( $context = null ) {
		...
		if ( ! empty( $resolved[0] ) ) {
			...
			return apply_filters( 'meyvc_offer_best_offer', $payload, $context );
		}
		return apply_filters( 'meyvc_offer_best_offer', null, $context );
	}
	...
	public static function get_rate_limit_hours() {
		return (int) apply_filters( 'meyvc_offer_coupon_rate_limit_hours', 6 );
	}
	...
	public static function get_coupon_ttl_hours() {
		return (int) apply_filters( 'meyvc_offer_coupon_ttl_hours', 48 );
	}
	...
	public static function get_default_coupon_ttl_days() {
		return (int) apply_filters( 'meyvc_offer_coupon_ttl_days', 2 );
	}
	...
	public static function get_offer_by_id( $id ) {
		...
		return class_exists( 'MEYVC_Offer_Model' ) ? MEYVC_Offer_Model::get( $id ) : null;
	}
	...
	public static function get_recent_log_for_visitor_offer( $offer_id, $visitor_id ) {
		if ( ! class_exists( 'MEYVC_Offer_Model' ) || empty( $visitor_id ) ) {
			return null;
		}
	...
	public static function create_coupon_for_offer( $offer, array $context ) {
		...
		$coupon_args = apply_filters( 'meyvc_offer_coupon_args', $coupon_args, $offer, $context );
		...
		update_post_meta( $coupon_id, '_meyvc_offer_id', $offer_id );
		update_post_meta( $coupon_id, '_meyvc_visitor_id', $visitor_id );
		update_post_meta( $coupon_id, '_meyvc_user_id', $user_id );
	...
	public static function get_or_create_coupon_for_offer( $offer, array $context ) {
		...
		do_action( 'meyvc_offer_coupon_generated', $code, $offer_row, $context );
