<?php
/**
 * Offer model: CRUD for meyvc_offers and audit log for meyvc_offer_logs.
 *
...
/**
 * MEYVC_Offer_Model class.
 */
class MEYVC_Offer_Model {
	...
	/**
	 * Use MEYVC_Offer_Model::from_row( $row ) for conflict helpers.
	 *
	 ...
	 *
	 * @param object $row Row from MEYVC_Offer_Model::get() or list entry.
	 * @return self
	 */
	...
	public static function get_offers_table() {
		return MEYVC_Database::get_table( 'offers' );
	}
	...
	public static function get_logs_table() {
		return MEYVC_Database::get_table( 'offer_logs' );
	}
	...
	public static function get( $id ) {
		...
		$oid   = absint( $id );
		$cache_key   = 'meyvora_meyvc_' . md5( serialize( array( 'offer_row_by_id', $table, $oid ) ) );
		$cached_row  = wp_cache_get( $cache_key, 'meyvora_meyvc' );
		if ( false === $cached_row ) {
	...
	public static function get_all( $status = null ) {
		...
		if ( $status !== null && $status !== '' ) {
			...
			$cache_key = 'meyvora_meyvc_' . md5( serialize( array( 'offer_list_by_status', $table, $status_s ) ) );
			...
			$cache_key = 'meyvora_meyvc_' . md5( serialize( array( 'offer_list_all_ordered', $table ) ) );
			$rows      = wp_cache_get( $cache_key, 'meyvora_meyvc' );
			if ( false === $rows ) {
				$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above.
					$wpdb->prepare(
	...
	public static function create( $data ) {
		...
		// MEYVC_Database::insert/sanitize_data will encode arrays to JSON.
		return MEYVC_Database::insert( $table, $data );
	}
	...
	public static function update( $id, $data ) {
		...
		// MEYVC_Database::update/sanitize_data will encode arrays to JSON.
		return MEYVC_Database::update( $table, $data, array( 'id' => $id ), null, array( '%d' ) );
	}
	...
	public static function delete( $id ) {
		...
		return MEYVC_Database::delete( $table, array( 'id' => $id ), array( '%d' ) );
	}
	...
	public static function log_insert( $offer_id, $visitor_id = null, $user_id = null, $coupon_code = null, $applied_at = null, $order_id = null ) {
		...
		$result = MEYVC_Database::insert( $table, $data );
		if ( $result && function_exists( 'do_action' ) ) {
			do_action( 'meyvc_offer_log_inserted', $offer_id );
		}
		return $result;
	}
	...
	public static function get_logs( $offer_id = null, $limit = 100 ) {
		...
		if ( $offer_id !== null && $offer_id !== '' ) {
			$oid       = absint( $offer_id );
			$cache_key = 'meyvora_meyvc_' . md5( serialize( array( 'offer_logs_by_offer_id', $table, $oid, $limit ) ) );
			$rows      = wp_cache_get( $cache_key, 'meyvora_meyvc' );
			if ( false === $rows ) {
				$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above.
					$wpdb->prepare(
			...
			$cache_key = 'meyvora_meyvc_' . md5( serialize( array( 'offer_logs_recent_all', $table, $limit ) ) );
			$rows      = wp_cache_get( $cache_key, 'meyvora_meyvc' );
			if ( false === $rows ) {
				$rows = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above.
					$wpdb->prepare(
