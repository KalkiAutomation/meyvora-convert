...
/**
 * CRO_Sequence_Engine class.
 */
class CRO_Sequence_Engine {
	...
	public function __construct() {
		global $wpdb;
		$t            = $wpdb->prefix . 'cro_sequence_enrollments';
		$cache_key    = 'meyvora_cro_' . md5( serialize( array( 'sequence_enrollments_table_exists', $t ) ) );
		$table_exists = wp_cache_get( $cache_key, 'meyvora_cro' );
		if ( false === $table_exists ) {
			...
			wp_cache_set( $cache_key, $table_exists, 'meyvora_cro', 300 );
		}
		if ( $table_exists !== $t ) {
			return;
		}

		add_action( 'cro_event_tracked', array( $this, 'on_event_tracked' ), 25, 1 );
		add_filter( 'cro_decision_active_campaigns', array( $this, 'inject_pending_campaign' ), 8, 3 );
	}

	/**
	 * @param array $event Event payload from CRO_Tracker.
	 */
	public function on_event_tracked( $event ) {
		...
		$key = class_exists( 'CRO_Tracker' ) ? CRO_Tracker::client_session_id() : '';
		if ( $key === '' ) {
			return;
		}

		global $wpdb;
		$seq_table = $wpdb->prefix . 'cro_campaign_sequences';
		$en_table  = $wpdb->prefix . 'cro_sequence_enrollments';

		$cache_key_seq = 'meyvora_cro_' . md5( serialize( array( 'sequences_active_by_trigger', $seq_table, 'active', $campaign_id ) ) );
		$sequences     = wp_cache_get( $cache_key_seq, 'meyvora_cro' );
		if ( false === $sequences ) {
			...
			wp_cache_set( $cache_key_seq, $sequences, 'meyvora_cro', 300 );
		} else {
		...
		foreach ( $sequences as $seq ) {
			...
			$cache_key_ex = 'meyvora_cro_' . md5( serialize( array( 'enrollment_id_by_visitor', $en_table, $sid, $key, 'active' ) ) );
			$exists       = wp_cache_get( $cache_key_ex, 'meyvora_cro' );
			if ( false === $exists ) {
				...
				wp_cache_set( $cache_key_ex, $exists, 'meyvora_cro', 300 );
			}
			if ( $exists ) {
				continue;
			}
			...
			if ( false !== $ins ) {
				if ( class_exists( 'CRO_Database' ) ) {
					CRO_Database::invalidate_table_cache_after_write( $en_table );
				}
	...
	private function maybe_advance_on_impression( array $event ) {
		...
		$cache_key_row = 'meyvora_cro_' . md5( serialize( array( 'enrollment_pending_impression', $en_table, $seq_table, 'active', $key, 'active', $campaign_id ) ) );
		$row           = wp_cache_get( $cache_key_row, 'meyvora_cro' );
		if ( false === $row ) {
			...
			wp_cache_set( $cache_key_row, $row, 'meyvora_cro', 300 );
		}
		...
		$cache_key_seq_row = 'meyvora_cro_' . md5( serialize( array( 'campaign_sequence_by_id', $seq_table, (int) $row->sequence_id ) ) );
		$seq               = wp_cache_get( $cache_key_seq_row, 'meyvora_cro' );
		if ( false === $seq ) {
			...
			wp_cache_set( $cache_key_seq_row, $seq, 'meyvora_cro', 300 );
		}
		if ( ! $seq ) {
			return;
		}
	 ...
	 *
	 * @param CRO_Campaign_Model[] $campaigns     Active campaigns.
	 * @param CRO_Context           $context       Context.
	 * @param CRO_Visitor_State     $visitor_state Visitor.
	...
	public function inject_pending_campaign( $campaigns, $context, $visitor_state ) {
		...
		$key = class_exists( 'CRO_Tracker' ) ? CRO_Tracker::client_session_id() : '';
		if ( $key === '' || ! class_exists( 'CRO_Campaign' ) || ! class_exists( 'CRO_Campaign_Model' ) ) {
			return $campaigns;
		}
		...
		$cache_key_due = 'meyvora_cro_' . md5( serialize( array( 'enrollment_due_inject', $en_table, $seq_table, 'active', $key, 'active', $now ) ) );
		$row           = wp_cache_get( $cache_key_due, 'meyvora_cro' );
		if ( false === $row ) {
			$row = $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above.
				$wpdb->prepare(
					'SELECT e.* FROM %i e
			...
			wp_cache_set( $cache_key_due, $row, 'meyvora_cro', 300 );
		}
		...
		$db  = CRO_Campaign::get( $cid );
		if ( ! is_array( $db ) || empty( $db['id'] ) ) {
			return $campaigns;
		}
		$model = CRO_Campaign_Model::from_db_row( $db );
	...
	public static function get_all() {
		global $wpdb;
		$t = $wpdb->prefix . 'cro_campaign_sequences';
		$cache_key_te = 'meyvora_cro_' . md5( serialize( array( 'campaign_sequences_table_exists', $t ) ) );
		$table_exists = wp_cache_get( $cache_key_te, 'meyvora_cro' );
		if ( false === $table_exists ) {
			$table_exists = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Cached above.
				$wpdb->prepare( 'SHOW TABLES LIKE %s', $t )
			);
			wp_cache_set( $cache_key_te, $table_exists, 'meyvora_cro', 300 );
		}
		if ( $table_exists !== $t ) {
			return array();
		}
		$cache_key_all = 'meyvora_cro_' . md5( serialize( array( 'campaign_sequences_all_rows', $t ) ) );
		$rows          = wp_cache_get( $cache_key_all, 'meyvora_cro' );
		if ( false === $rows ) {
			...
			wp_cache_set( $cache_key_all, $rows, 'meyvora_cro', 300 );
		} else {
	...
	public static function save( $data ) {
		global $wpdb;
		$table = $wpdb->prefix . 'cro_campaign_sequences';
	...
	public static function delete( $id ) {
		...
		$en_table = $wpdb->prefix . 'cro_sequence_enrollments';
		$seq_tbl  = $wpdb->prefix . 'cro_campaign_sequences';
		...
		if ( class_exists( 'CRO_Database' ) ) {
			CRO_Database::invalidate_table_cache_after_write( $seq_tbl );
		}
		self::sequence_flush_read_cache();
	}
}
