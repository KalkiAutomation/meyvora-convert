...
(function () {
	...
	const STORAGE_KEY = 'meyvc_cart_exit_nudge_shown';
	...
	function showNudge( config ) {
		...
		overlay.className = 'meyvc-exit-nudge';
		overlay.setAttribute( 'role', 'dialog' );
		overlay.setAttribute( 'aria-label', config.message );

		const box = document.createElement( 'div' );
		box.className = 'meyvc-exit-nudge__box';

		const p = document.createElement( 'p' );
		p.className = 'meyvc-exit-nudge__message';
		p.textContent = config.message;

		const cta = document.createElement( 'a' );
		cta.href = config.checkoutUrl || '#';
		cta.className = 'meyvc-exit-nudge__cta button';
		...
		close.type = 'button';
		close.className = 'meyvc-exit-nudge__close';
		...
		function closeNudge() {
			overlay.classList.remove( 'meyvc-exit-nudge--visible' );
			setTimeout( function () {
		...
		requestAnimationFrame( function () {
			overlay.classList.add( 'meyvc-exit-nudge--visible' );
		} );
	}
	...
	if ( typeof window.meyvcCartExitNudgeConfig !== 'undefined' ) {
		init( window.meyvcCartExitNudgeConfig );
	}
})();
