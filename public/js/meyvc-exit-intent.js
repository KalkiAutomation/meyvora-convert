...
(function($) {
	...
	function triggerExitIntent(source) {
		...
		$(document).trigger('meyvc:exit-intent', source ? [source] : []);
	}
	...
	window.meyvcSignals = new MEYVCSignalCollector();
	...
	class MEYVCDebugger {
		...
		_isOn() {
			return typeof window.MEYVCDebug !== 'undefined' && window.MEYVCDebug.isEnabled();
		}
		...
		log(category, message, data = null) {
			...
			window.MEYVCDebug.log(`[CRO Debug] ${category}: ${message}`, data || '');
		}
		...
		render() {
			...
			const $header = $('<div>', {
				...
				html: '🔧 Meyvora Convert Debug <button id="meyvc-debug-close" style="float:right;background:none;border:none;color:#fff;cursor:pointer;">×</button>'
			});

			const $content = $('<div>', { id: 'meyvc-debug-content' });

			$panel.append($header).append($content);
			$('body').append($panel);

			$('#meyvc-debug-close').on('click', () => $panel.hide());

			this.updatePanel();
		}

		updatePanel() {
			const $content = $('#meyvc-debug-content');
		...
		checkBehavior(rules, context) {
			...
			const behavior = window.meyvcBehavior || {};
		...
		checkFrequency(campaign) {
			...
			const cookieName = 'meyvc_campaign_' + campaignId;
	...
	// Initialize debugger (panel only when debug mode is on — same gate as MEYVCDebug).
	const meyvcDebugger = new MEYVCDebugger();
	if (window.MEYVCDebug && window.MEYVCDebug.isEnabled()) {
		$(document).ready(function() {
			meyvcDebugger.render();
		});
	}

	// Expose debugger globally for use by popup scripts
	window.meyvcDebugger = meyvcDebugger;

	$(document).ready(function() {
		var sig = window.meyvcSignals;
		...
		if (isMobile && sig) {
			...
			history.pushState({ meyvc_exit_guard: true }, '');
