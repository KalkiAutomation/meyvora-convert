 ...
 * Dispatches 'meyvc:signal' when signals change. Uses throttling for performance.
 *
...
(function() {
	...
	/**
	 * MEYVCSignalCollector – collects UX signals for exit-intent and intent scoring.
	 */
	class MEYVCSignalCollector {
		...
		dispatchSignal() {
			...
			try {
				document.dispatchEvent(new CustomEvent('meyvc:signal', {
	...
	var collector = new MEYVCSignalCollector();
	window.MEYVCSignalCollector = MEYVCSignalCollector;
	window.meyvcSignals = collector;
	window.meyvcSignalCollector = collector;
